...
\begin{itemize}
	\item \(n_{\vec R_0}\rho n_{\vec R_0}\) + \(n_{\vec R_0 + \vec a}\rho n_{\vec R_0 + \vec a}\)
	\item \(e^{i \vec{q}\cdot\vec{a}}c^\dagger_{\vec R_0 + \vec a}c_{\vec R_0}\rho n_{\vec R_0}\) + h.c. + 
	      \(e^{-i \vec{q}\cdot\vec{a}}c^\dagger_{\vec R_0}c_{\vec R_0 + \vec a}\rho n_{\vec R_0}\) + h.c.
	\item \(c^\dagger_{\vec R_0 + \vec a}c_{\vec R_0 + \vec a}\rho n_{\vec R_0}\) + h.c.
	\item   \(e^{2i\vec{q}\cdot\vec{a}}c^\dagger_{\vec R_0 + \vec a}c_{\vec R_0}\rho c^\dagger_{\vec R_0 + \vec a}c_{\vec R_0}\) + h.c. + 
		\(c^\dagger_{\vec R_0}c_{\vec R_0 + \vec a}\rho c^\dagger_{\vec R_0 + \vec a}c_{\vec R_0}\) + h.c.
	\item \(e^{i \vec{q}\cdot\vec{a}} n_{\vec R_0 + \vec a}\rho c^\dagger_{\vec R_0 + \vec a}c_{\vec R_0}\) + h.c. + \(e^{-i \vec{q}\cdot\vec{a}} n_{\vec R_0 + \vec a}\rho c^\dagger_{\vec R_0}c_{\vec R_0 + \vec a}\) + h.c.
\end{itemize}
...
\end{aligned}\end{equation}
By taking the outer product with its hermitian conjugate, we get the reduced density matrix:
\begin{equation}\begin{aligned}
	\rho_{\vec k\sigma} \simeq \left[\hat n_{d,\sigma} + \hat n_{z,\sigma} + \frac{\xi_{\vec k}}{\sqrt \mathcal{W}}\left(c^\dagger_{d,\sigma}c_{z,\sigma} + \text{h.c.} \right)\right] \rho_\text{aux} \left[\hat n_{d,\sigma} + \hat n_{z,\sigma} + \frac{\xi_{\vec k}}{\sqrt \mathcal{W}}\left(c^\dagger_{d,\sigma}c_{z,\sigma} + \text{h.c.} \right)\right]\label{density_matrix_kspace}
\end{aligned}\end{equation}
...
Another measure of many-body correlations would be the mutual information between two momentum-space states. The mutual information \(I_2\) between the states \(\ket{\vec k, \sigma}\) and \(\ket{-\vec k, \sigma}\) is defined as
\begin{equation}\begin{aligned}
	I_2(\vec k\sigma:-\vec k\sigma) = S_\text{EE}(\rho_{\vec k\sigma}) + S_\text{EE}(\rho_{-\vec k\sigma}) - S_\text{EE}(\rho_{\vec k\sigma, -\vec k\sigma})~,
\end{aligned}\end{equation}
where \(S_\text{EE}(\rho)\) is the von-Neumann entropy associated with the density matrix \(\rho\). The reduced density matrices \(\rho_{\pm\vec k\sigma}\) can be computed using eq.~\ref{density_matrix_kspace}; the reduced density matrix \(\rho_{\vec k\sigma, \vec k\sigma}\) has both the states \(\pm \vec k\) traced out, and it can be obtained by substituting \(\rho_{\vec k\sigma}\) back into eq.~\ref{density_matrix_kspace}:
\begin{equation}\begin{aligned}
	\rho_{\vec k\sigma, -\vec k\sigma} &= \hat n_{-\vec k\sigma}\rho_{\vec k\sigma}\hat n_{-\vec k\sigma} + \hat h_{-\vec k\sigma}\rho_{\vec k\sigma}\hat h_{-\vec k\sigma} \\
					   &= \left[\hat n_{d,\sigma} + \hat n_{z,\sigma} + \frac{\xi_{-\vec k}}{\sqrt \mathcal{W}}\left(c^\dagger_{d,\sigma}c_{z,\sigma} + \text{h.c.} \right)\right] \rho_{\vec k\sigma} \left[\hat n_{d,\sigma} + \hat n_{z,\sigma} + \frac{\xi_{-\vec k}}{\sqrt \mathcal{W}}\left(c^\dagger_{d,\sigma}c_{z,\sigma} + \text{h.c.} \right)\right]
\end{aligned}\end{equation}
