...
\begin{equation}\begin{aligned}
	...
	=& \left(1 + \xi_{\vec k}^2 + \Xi_{\vec k}^2\right)|\braket{n_{\vec R_0,\sigma} | \Psi_\text{gs}}|^2 + 2\xi_{\vec k}\left(1 + \Xi_{\vec k}\right)\braket{\Psi_\text{gs} | n_{\vec R_0,\sigma}}\braket{n_{\vec R_0 + \vec a,\sigma} | \Psi_\text{gs}} \\
							      & + 2\Xi_{\vec k}\braket{\Psi_\text{gs} | n_{\vec R_0,\sigma}}\braket{n_{\vec R_0 + \vec a^\prime,\sigma} | \Psi_\text{gs}} 
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	|\braket{\Psi_\text{gs} | n_{\vec k,\sigma}}|^2 \simeq& \frac{1 + \xi_{\vec k}^2 + \Xi_{\vec k}^2}{3\lambda_{\vec k_0}^2}\left[\braket{\psi_\text{aux} | n_{d,\sigma}}\braket{n_{d,\sigma} | \psi_\text{aux}} + \frac{1}{\mathcal{W}}\braket{\psi_\text{aux} | n_{z,\sigma}}\braket{n_{z,\sigma} | \psi_\text{aux}} + \frac{1}{\mathcal{W}}\braket{\psi_\text{aux} | n_{1,\sigma}}\braket{n_{1,\sigma} | \psi_\text{aux}}\right]\\
							      &+ \frac{2\xi_{\vec k}\left(1 + \Xi_{\vec k}\right)}{4\lambda_{\vec k_0}^2 \mathcal{W}}\left[\braket{\psi_\text{aux} \left(\ket{n_{d,\sigma}}\bra{n_{z,\sigma}} + \ket{n_{z,\sigma}}\bra{n_{1,\sigma}} + \ket{n_{z,\sigma}}\bra{n_{d,\sigma}} + \ket{n_{1,\sigma}}\bra{n_{z,\sigma}}\right) \psi_\text{aux}}\right] \\
							      &+ \frac{2\Xi_{\vec k}}{2\lambda_{\vec k_0}^2 \mathcal{W}}\left[\braket{\psi_\text{aux} \left(\ket{n_{d,\sigma}}\bra{n_{1,\sigma}} + \ket{n_{1,\sigma}}\bra{n_{d,\sigma}}\right) \psi_\text{aux}}\right]
\end{aligned}\end{equation}
The \(k-\)space reduced density matrix takes the form
\begin{equation}\begin{aligned}
	\rho_{\vec k,\sigma} =& \sum_{n_{(d,z,1),\sigma}=0,1}\frac{1 + \xi_{\vec k}^2 + \Xi_{\vec k}^2}{3\lambda_{\vec k_0}^2}\left[\bar\psi^\dagger_{n_{d\sigma}}\bar\psi_{n_{d\sigma}} + \frac{1}{\mathcal{W}}\bar\psi^\dagger_{n_{z\sigma}}\bar\psi_{n_{z\sigma}} + \frac{1}{\mathcal{W}}\bar\psi^\dagger_{n_{1\sigma}}\bar\psi_{n_{1\sigma}}\right] + \sum_{n_{(d,1),\sigma}=0,1}\frac{\Xi_{\vec k}}{\lambda_{\vec k_0}^2 \mathcal{W}}\left[\bar\psi^\dagger_{n_{1\sigma}}\bar\psi_{n_{d\sigma}} + \bar\psi^\dagger_{n_{d\sigma}}\bar\psi_{n_{1\sigma}}\right]\\
							      &+ \sum_{n_{(d,z,1),\sigma}=0,1}\frac{\xi_{\vec k}\left(1 + \Xi_{\vec k}\right)}{2\lambda_{\vec k_0}^2 \mathcal{W}}\left[\bar\psi^\dagger_{n_{d\sigma}}\bar\psi_{n_{z\sigma}} + \bar\psi^\dagger_{n_{z\sigma}}\bar\psi_{n_{d\sigma}} + \bar\psi^\dagger_{n_{1\sigma}}\bar\psi_{n_{z\sigma}} + \bar\psi^\dagger_{n_{z\sigma}}\bar\psi_{n_{1\sigma}}\right]
\end{aligned}\end{equation}
where \(\bar\psi_{n_{d\sigma}} = \braket{n_{d,\sigma} | \psi_\text{aux}}\) and similar definitions exist for \(\bar\psi_{n_{z\sigma}}\) and \(\bar\psi_{n_{1\sigma}}\). The von-Neumann entropy \(S_\text{EE}(\vec k)\) can then be calculated by taking the trace of the operator \(\rho_{\vec k,\sigma}\ln \rho_{\vec k,\sigma}\).
