...
We now focus on the quantum critical point \(g = -4u\) of our model. At this point, the phase shifts get related to each other: \(\delta^*_\text{sp} = -4\delta^*_\text{ch}\), so that the exponent of the self-energy takes the form
	...
	\begin{equation}\begin{aligned}
		\Sigma_{dd} \sim |\omega|^{\gamma_{dd}}\simeq |\omega|^{1 + \frac{3}{8}\alpha}
	\end{aligned}\end{equation}
The power-law exponent $\gamma_{dd}=1 + \frac{3}{8}\alpha$ lies in the following range: $1\leq \gamma_{dd} \leq 2$. This can be compared with the expression \(\sim|\omega|^{2-3\alpha}\) of the impurity self-energy obtained in ref.~\cite{fritzvojta2004} where the power law exponent ranges between $1/2$ and $2$. In a similar manner, the self-energy \(\Sigma_{d0}\) for scattering processes that connect the impurity site and the bath zeroth site can also be obtained. The exponent \(\gamma_{d0}\) for this self-energy is given by \(\gamma_{d0} = 2 - 2\gamma_{dd}\), so that this non-local self-energy diverges as
	\begin{equation}\begin{aligned}
		\Sigma_{d0} \sim |\omega|^{- \frac{3}{4}\alpha}~.
	\end{aligned}\end{equation}
\par 
By using our tiling approach, the \(k-\)space self-energy at the critical point can be written as
	\begin{equation}\begin{aligned}
		\Sigma(\vec k,\omega) = \Sigma_{dd} + \frac{1}{\mathcal{W}}\xi_{\vec k}\Sigma_{d0}
	\end{aligned}\end{equation}
	%This expression suggests that at the critical point, all points in the Brillouin zone except those at the Fermi surface get gapped out. 
	It is clear that the divergent $\Sigma_{d0}$ does not affect the single-particle self-energy $\Sigma(\vec k,\omega)$ precisely at the Fermi surface, i.e., for $\xi_{\vec{k}}= 0$; $\Sigma(\vec k,\omega)$ diverges everywhere else in the Brillouin zone. This corresponds to a gapless non-Fermi liquid metal present at the QCP, corresponding to a system with Landau quasiparticle poles precisely on the Fermi surface but which are destroyed everywhere else. Now, following the arguments laid out in Ref.\cite{sujan2023}, the single-particle spectral function $A(\omega,\vec{k})$ will vanish (i.e., be pseudogapped) for excitations away from the Fermi surface
\begin{equation}
...
%This guarantees that the system at the QCP remains gapless, and corresponds to a ``local quantum critical" metal~\cite{Si2001}.	
