...
\begin{equation}\begin{aligned}
	...
	\mathcal{H}_\text{full} &= \sum_{{\bf r}}\mathcal{H}_\text{aux}({\bf r})\\
				&= \sum_{{\bf r}}T^\dagger({\bf r} - {\bf r}_d)\mathcal{H}_\text{aux}({\bf r}_d)T({\bf r} - {\bf r}_d)~ \\
				&= \frac{1}{\mathcal{Z}}\sum_{{\bf r}}\sum_{{\bf z}_1\in\text{NN}({\bf r}_d)}T^\dagger({\bf r} - {\bf r}_d)\mathcal{H}_\text{aux}({\bf r}_d, {\bf z}_1)T({\bf r} - {\bf r}_d)~ \\
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
The interaction Hamiltonian can also be generalised to:
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{imp-cbath} &= \sum_{{\bf z}_i,{\bf z}_j}\sum_{\alpha,\alpha^\prime}\frac{1}{ Z}J({\bf z}_i - {\bf z}_j) {\bf S}_{{\bf r}_d}\cdot{\boldsymbol \sigma}_{\alpha,\alpha^\prime}c^\dagger_{{\bf z}_i,\alpha}c_{{\bf z}_j,\alpha^\prime} \\
				     &+  \sum_{\sigma, {\bf z}_i} \frac{1}{\sqrt Z}V\left({\bf r}_d - {\bf z}_i\right) \left(c^\dagger_{{\bf r}_d,\sigma} c_{{{\bf z}_i}\sigma} + h.c.\right)
\end{aligned}\end{equation}
where \({\bf z}_i\) and \({\bf z}_j\) sum over the bath zeroth sites of the impurity model, and \(\boldsymbol \sigma\) is the vector of Pauli matrices.% The bath zeroth sites, being locally connected to the impurity, typically reside on the sites that are nearest to the impurity. This means that we can write \({\bf r}_0 = {\bf r}_d + {\bf a}\), where \({\bf a}\) is a primitive lattice vector that connects \({\bf r}_d\) with its nearest neighbours.


We now follow the prescription laid out in eq.~\ref{tilingPrescription}. The tiled Hamiltonian can be written as 
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{full} = \frac{1}{\mathcal{Z}}\sum_{{\bf r}}\sum_{{\bf z}_1\in\text{NN}({\bf r}_d)}T^\dagger({\bf r} - {\bf r}_d)\left[\mathcal{H}_\text{cbath} + \mathcal{H}_\text{imp} \right.\\
...
\begin{equation}\begin{aligned}
	&\frac{1}{\mathcal{Z}}\sum_{{\bf r}}\sum_{{\bf z}_1\in\text{NN}({\bf r}_d)}T^\dagger({\bf r} - {\bf r}_d)\mathcal{H}_\text{cbath}({\bf r}_d, {\bf z}_1)T({\bf r} - {\bf r}_d) \\
	=& \sum_{{\bf r}}T^\dagger({\bf r} - {\bf r}_d)\left[-t\sum_{\left<{\bf r}_i, {\bf r}_j\right>\neq{\bf r}_d;\sigma}\left(c^\dagger_{{\bf r}_i,\sigma}c_{{\bf r}_j,\sigma} + \text{h.c.}\right)\right.\\
	 &\left.- \sum_{{\bf z}_1\in\text{NN}({\bf r}_d)}\sum_{{\bf z}_i}\frac{U_b({\bf z}_i)}{2Z \mathcal{Z}}\left(\hat n_{{\bf z}_i, \uparrow} - \hat n_{{\bf z}_i, \downarrow}\right)^2\right]T({\bf r} - {\bf r}_d) \\
	=& -t\sum_{\left<{\bf r}_i, {\bf r}_j\right>;\sigma}\sum_{{\bf r}\neq{\bf r}_i,{\bf r}_j}\left(c^\dagger_{{\bf r}_i,\sigma}c_{{\bf r}_j,\sigma} + \text{h.c.}\right) \\
	 &- \frac{1}{2\mathcal{Z}Z}\sum_{\bf r}\sum_{{\bf z}_1\in\text{NN}\left({\bf r}\right) }\sum_{{\bf z}_i \in \text{NN}({\bf r})} U_b({\bf z}_i)  \left(\hat n_{{\bf z}_i, \uparrow} - \hat n_{{\bf z}_i, \downarrow}\right)^2 \\
	=& -t(N-2)\sum_{\left<{\bf r}_i, {\bf r}_j\right>;\sigma}\left(c^\dagger_{{\bf r}_i,\sigma}c_{{\bf r}_j,\sigma} + \text{h.c.}\right)\\
	 &- \frac{1}{2}\overline{U_b}\sum_{\bf r}\left(\hat n_{{\bf r} , \uparrow} - \hat n_{{\bf r} , \downarrow}\right)^2\\
\end{aligned}\end{equation}
where \(\overline{U_b} = \sum_{\bf a} \frac{1}{z}U_b({\bf a})\) is the average of \(U_b({\bf a})\) over the bath zeroth sites in the auxliary model. The inequality \(\left<{\bf r}_i, {\bf r}_j\right>\neq{\bf r}\) ensures that each nearest-neighbour pair appears in \(N-2\) instances of the auxiliary model, \(N\) being the total number of lattice sites. The two instances that do not contribute are the ones in which the impurity site itself is at \({\bf r}_i\) or \({\bf r}_j\). In the first step, the factor of \(\mathcal{Z}\) is cancelled out by the trivial sum over \({\bf r}_0\).
...
\begin{widetext}
\begin{equation}\begin{aligned}
	&\frac{1}{\mathcal{Z}}\sum_{{\bf r}}\sum_{{\bf r}_0\in\text{NN}({\bf r}_d)}T^\dagger({\bf r} - {\bf r}_d)\mathcal{H}_\text{cbath}T({\bf r} - {\bf r}_d)\\
	&=\frac{1}{\mathcal{Z}}\sum_{{\bf r}}\sum_{{\bf r}_0\in\text{NN}({\bf r}_d)}T^\dagger({\bf r} - {\bf r}_d)\left[\sum_{{\bf a},{\bf a}^\prime}\sum_{\alpha,\alpha^\prime}\frac{1}{Z}J({\bf a} - {\bf a}^\prime) {\bf S}_{{\bf r}_d}\cdot{\boldsymbol \sigma}c^\dagger_{{\bf r}_d + {\bf a},\alpha}c_{{\bf r}_d + {\bf a}^\prime,\alpha^\prime} + \sum_{{\bf a}, \sigma} \frac{1}{\sqrt Z}V\left({\bf a}\right) \left(c^\dagger_{{\bf r}_d,\sigma} c_{{{\bf r}_d + {\bf a}},\sigma} + h.c.\right)\right]T({\bf r} - {\bf r}_d)
\end{aligned}\end{equation}
\end{widetext}
\clearpage
...
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{aux}(i,j) = -2t\sum_{<m,n>,\sigma}^{m \neq i \neq n} \left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) - \frac{U}{2}\left( \hat n_{i \uparrow} - \hat n_{i \downarrow} \right) ^2 \\
	+ V \sum_{\sigma} \left(c^\dagger_{j\sigma} c_{i\sigma} + h.c.\right) +J \vec{S_i}\cdot\vec{S}_j - \frac{1}{2}U_b\left(\hat n_{j \uparrow} - \hat n_{j \downarrow}\right)^2 
\end{aligned}\end{equation}
The tight-binding term sums over all nearest pairs \((m,n)\) that do not involve the site \(i\).
In general \(i\) will have \(\mathcal{Z}\) nearest neighbours for a lattice with coordination number \(\mathcal{Z}\). The total local Hamiltonian for the site \(i\) is obtained by averaging over the Hamiltonians \(\mathcal{H}_\text{aux}(i,j)\) for all nearest neighbours \(j\) of \(i\):
\begin{widetext}
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{aux}(i) &= \frac{1}{\mathcal{Z}}\sum_{j \in \text{NN of i}} \mathcal{H}_\text{aux}(i,j) = - \frac{U}{2}\left( \hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 + \frac{1}{\mathcal{Z}}\sum_{j \in \text{NN of i}}\left[-2t\sum_{<m,n>,\sigma}^{m \neq i \neq n}\left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) + V \sum_{\sigma} \left(c^\dagger_{j\sigma} c_{i\sigma} + h.c.\right)  \right.\\
				  &\left. +J \vec{S_i}\cdot\vec{S}_j - \frac{1}{2}U_b\left(\hat n_{j \uparrow} - \hat n_{j \downarrow}\right)^2\right]\\
				  &= - \frac{U}{2}\left( \hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 -2t\sum_{<m,n>,\sigma}^{m \neq i \neq n}\left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) +  \frac{1}{\mathcal{Z}}\sum_{j \in \text{NN of i}}\left[V \sum_{\sigma} \left(c^\dagger_{j\sigma} c_{i\sigma} + h.c.\right) +J \vec{S_i}\cdot\vec{S}_j - \frac{1}{2}U_b\left(\hat n_{j \uparrow} - \hat n_{j \downarrow}\right)^2\right]
\end{aligned}\end{equation}
Once we have this local Hamiltonian for a particular site, we translate this over all sites \(i\):
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{full} =& \sum_i \mathcal{H}_\text{aux}(i) = \sum_i\left[- \frac{U}{2}\left( \hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 -2t\sum_{<m,n>,\sigma}^{m \neq i \neq n}\left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) +  \frac{1}{\mathcal{Z}}\sum_{j \in \text{NN of i}}\left[V \sum_{\sigma} \left(c^\dagger_{j\sigma} c_{i\sigma} + h.c.\right) \right.\right.\\
				  &\left.\left. + J \vec{S_i}\cdot\vec{S}_j - \frac{1}{2}U_b\left(\hat n_{j \uparrow} - \hat n_{j \downarrow}\right)^2\right]\right]\\
	=& -\frac{1}{2}\left(U + U_b\right) \sum_i \left( \hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 + \sum_i\left[-2t\sum_{<m,n>,\sigma}^{m \neq i \neq n}\left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) +  \frac{1}{\mathcal{Z}}\sum_{j \in \text{NN of i}}\left[V \sum_{\sigma} \left(c^\dagger_{j\sigma} c_{i\sigma} + h.c.\right) + J \vec{S_i}\cdot\vec{S}_j\right]\right]
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{full} =& -\frac{1}{2}\left(U + U_b\right) \sum_i \left( \hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 - 2t\left(N-2\right)\sum_{<m,n>,\sigma}\left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) + \frac{2}{\mathcal{Z}}\sum_{<i,j>}\left[\sum_\sigma V \left(c^\dagger_{j\sigma} c_{i\sigma} + h.c.\right) + J \vec{S_i}\cdot\vec{S}_j\right]\\
				  &=-\frac{1}{2}\left(U + U_b\right)\sum_i \left(\hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 - \left(2t (N-2) - \frac{2V}{\mathcal{Z}}\right) \sum_{<m,n>,\sigma}\left(c^\dagger_{m\sigma}c_{n\sigma} + \text{h.c.}\right) + \frac{2J}{\mathcal{Z}} \sum_{<i,j>} \vec{S_i}\cdot\vec{S}_j 
\end{aligned}\end{equation}
\end{widetext}
We end up with a Hubbard-Heisenberg model, of the form:
\begin{equation}\begin{aligned}
	\mathcal{H}_{H-H} = - \frac{1}{2}U_\text{H-H}\sum_{i} \left(\hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 + \frac{1}{\mathcal{Z}}J_\text{H-H}\sum_{<i,j>} \vec{S}_i\cdot\vec{S}_j\\
	- \frac{1}{\mathcal{Z}}t_\text{H-H}\sum_{<i,j>,\sigma}\left(c^\dagger_{i\sigma}c_{j\sigma} + \text{h.c.}\right) \\
\end{aligned}\end{equation}
where the bulk parameters are related to the auxiliary model couplings through the mapping:
\begin{equation}\begin{aligned}
	\label{map_aux_bulk}
	t_{H-H} &= \left(2t \mathcal{Z}(N-2) - 2V\right),\\
	U_{H-H} &= U + U_b,\\
	J_{H-H} &= 2J
\end{aligned}\end{equation}
...
From the form in eq.~\ref{tilingPrescription}, the tiled Hamiltonian is symmetric under discrete many-body translations:
\begin{equation}\begin{aligned}\label{translationSymmetry}
	T({\bf r})\mathcal{H}_\text{full}T^\dagger({\bf r}) = \sum_{{\bf r}^\prime}T({\bf r} + {\bf r}^\prime)\mathcal{H}_\text{aux}({\bf r_0})T^\dagger({\bf r} + {\bf r}^\prime) = \sum_{{\bf r}^\prime}T({\bf r}^\prime)\mathcal{H}_\text{aux}({\bf r_0})T^\dagger({\bf r}^\prime) = \mathcal{H}_\text{full}~.
\end{aligned}\end{equation}
There we used the fact that \({\bf r} + {\bf r}^\prime\) simply translates the auxiliary model defined by the coordinate \({\bf r}^\prime + {\bf r_0}\) to a new auxiliary model at the modified coordinate \({\bf r} + {\bf r}^\prime + {\bf r_0}\). Since this transformation from one auxiliary model to another simply shuffles the group of all auxiliary models, the sum over the group remains the sum, and we recover the same bulk model.

The fact that the Hamiltonian \(\mathcal{H}_\text{full}\) commutes with the many-body translation operator implies that the total momentum \(\vec k\) is a conserved quantity. This is in fact because the many-body translation operators can be obtained in terms of \(\vec k\): \(T({\bf r}) = \exp\left\{ i \vec{k}\cdot\vec{r} \right\} \).

\subsection{Moving away from half-filling}
...
\begin{equation}\begin{aligned}
	\mathcal{H}_{H-H} = -\frac{1}{2}U_\text{H-H} \sum_{i} \left(\hat n_{i \uparrow} - \hat n_{i \downarrow} \right)^2 - \frac{1}{\mathcal{Z}}t_\text{H-H}\sum_{<i,j>,\sigma}\left(c^\dagger_{i\sigma}c_{j\sigma} + \text{h.c.}\right) + \frac{1}{\mathcal{Z}}J_\text{H-H}\sum_{<i,j>} \vec{S}_i\cdot\vec{S}_j + \mu_\text{H-H}\sum_{i\sigma}\left(\hat n_{i\sigma} - \frac{1}{2}\right) 
\end{aligned}\end{equation}
...
In the tight-binding approach to lattice problems, the full Hamiltonian is described by adding the localised Hamiltonians at each site, and the full eigenstate \(\ket{\Psi}\) is then obtained by constructing liner combinations of the eigenstates \(\ket{\psi_i}\) of the local Hamiltonians such that \(\ket{\Psi}\) satisfies Bloch's theorem: \(\ket{\Psi_{\vec k}({\bf r})} = \sum_i e^{i \vec{k}\cdot\vec{R}_i} \ket{\psi_i({\bf r} - {\bf r}_i)}\). Bloch's theorem ensures that eigenstates satisfy the following relation under a translation operation \(T({\bf r})\) by some lattice distance \({\bf r} = \sum_i n_i {\bf a}_i\):
\begin{equation}\begin{aligned}
	T({\bf r})\ket{\Psi_{\vec k}\left({\bf r}\right)} = \ket{\Psi_{\vec k}({\bf r} + {\bf r})} = \sum_i e^{i \vec{k}\cdot\vec{R}_i} \ket{\psi_i({\bf r} + {\bf r} - {\bf r}_i)} = e^{i \vec{k}\cdot\vec{R}}\sum_i e^{i \vec{k}\cdot\left(\vec{R}_i - {\bf r}\right)} \ket{\psi_i({\bf r} - ({\bf r}_i - {\bf r}))} = e^{\vec{k}\cdot\vec{R}}\ket{\Psi({\bf r})}
\end{aligned}\end{equation}
At the end, we used the fact that \({\bf r}_i - {\bf r}\) is just another lattice vector \({\bf r}_j\). As mentioned before, these translation operators share eigenstates with the momentum operator:
\begin{equation}\begin{aligned}
	T\left( {\bf r} \right) \ket{\vec k} = \sum_{{\bf r}_i} e^{i \vec{k}\cdot\vec{r}_i}T\left( {\bf r} \right)\ket{{\bf r}_i} = e^{i \vec{k}\cdot\vec{R}}\sum_{{\bf r}_i} e^{i \vec{k}\cdot\left(\vec{r}_i - {\bf r}\right)}\ket{{\bf r}_i - {\bf r}} = e^{i \vec{k}\cdot\vec{R}} \ket{\vec k}
\end{aligned}\end{equation}

The eigenstates of bulk Hamiltonian obtained in eq.~\ref{siam_to_hubb} also enjoy a {\it many-body} Bloch's theorem~\cite{stoyanova}, because the tiling procedure restores the translation symmetry of the Hamiltonian (as shown in eq.~\ref{translationSymmetry}). This means that we can use the auxiliary model eigenstates
\begin{equation}\begin{aligned}
\psi^n_\text{aux}\left({\bf r}_i, {\bf a}\right)~,
\end{aligned}\end{equation}
with the impurity position \({\bf r}_i\) and zeroth site position \({\bf r}_i + {\bf a}\), to construct eigenstates of the bulk Hamiltonian. \({\bf r}_i\) is a lattice vector, while \({\bf a}\) is one of the \(\mathcal{Z}\) vectors that connect a lattice site with its nearest neighbours. The \(n\) in the superscript indicates that it is the \(n^\text{th}\) eigenstate of the auxiliary model. \(n=0\) is the ground state of the auxiliary model, and higher values denote the excited states. The following un-normalised combination of the auxiliary model eigenstates satisfies a many-particle equivalent of Bloch's theorem~\cite{stoyanova}:
\begin{equation}\begin{aligned}
	\ket{\Psi^n_{\vec k}} = \frac{1}{\sqrt{\mathcal{Z} N}}\sum_{{\bf r}_i, {\bf a}} e^{i \vec{k}\cdot\vec{R}_i} \psi^n_\text{aux}\left({\bf r}_i, {\bf a}\right)
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\label{bloch-states}
	\ket{\Psi_{\vec k}} = \frac{1}{\lambda_{\ket{\vec k}} \sqrt{\mathcal{Z} N}}\sum_{{\bf r}_i, {\bf a}} e^{i \vec{k}\cdot\vec{R}_i} \psi_\text{aux}\left({\bf r}_i, {\bf a}\right)~,
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\braket{\Psi_{\vec k} | \Psi_{\vec k}} &= \frac{1}{\mathcal{Z}^2 \lambda_{\vec k}^2 N^2}\sum_{{\bf r}, {\bf r}^\prime, a, a^\prime}e^{i \vec{k}\cdot({\bf r} - {\bf r}^\prime)}\braket{\psi_\text{aux}({\bf r}^\prime, {\bf a}^\prime) | \psi_\text{aux}({\bf r}, {\bf a})} = \frac{1}{\lambda_{\vec k}^2 N^2}\sum_{{\bf r}, {\bf r}^\prime, }e^{i \vec{k}\cdot({\bf r} - {\bf r}^\prime)}\braket{\psi_\text{aux}({\bf r}^\prime) | \psi_\text{aux}({\bf r})}\\
					       &=\frac{1}{\lambda_{\vec k}^2 N^2}\sum_{{\bf r}, {\bf r}^\prime}e^{i \vec{k}\cdot({\bf r} - {\bf r}^\prime)}\braket{\psi_\text{aux}({\bf r_0}) | T^\dagger({\bf r} - {\bf r}^\prime) | \psi_\text{aux}({\bf r_0})} = \frac{1}{\lambda_{\vec k}^2 N^2}\sum_{{\bf r}, {\bf r}^\prime, q}e^{i \vec{k}\cdot({\bf r} - {\bf r}^\prime)}\braket{\psi_\text{aux}({\bf r_0}) | T^\dagger({\bf r} - {\bf r}^\prime) \ket{\vec q}\bra{\vec q} \psi_\text{aux}({\bf r_0})}\\
					       &= \frac{1}{\lambda_{\vec k}^2 N^2}\sum_{{\bf r}, {\bf r}^\prime, q}e^{i (\vec{k} - \vec q)\cdot({\bf r} - {\bf r}^\prime)}\braket{\psi_\text{aux}({\bf r_0}) \ket{\vec q}\bra{\vec q} \psi_\text{aux}({\bf r_0})} = \frac{1}{\lambda_{\vec k}^2 N^2}\sum_{q} N^2 \delta_{\vec{k},\vec q}|\braket{\psi_\text{aux}({\bf r_0}) | \vec q}|^2\\
\end{aligned}\end{equation}
By requiring this norm to be unity, we obtain \(\lambda_{\vec k}^2 = |\braket{\psi_\text{aux}({\bf r_0}) | \vec k}|^2\). In the above exercise, we defined a state 
\begin{equation}\begin{aligned}\label{zeroth_site_average}
	\ket{\psi_\text{aux}({\bf r})} = (1/w)\sum_{{\bf a}}\ket{\psi_\text{aux}({\bf r}, {\bf a})}~,
\end{aligned}\end{equation}
by averaging over the neasest neighbours of \({\bf r}\), and inserted the total momentum basis \(1 = \sum_{\vec q}\ket{\vec q}\bra{\vec q}\) to resolve the translation operators. \({\bf r_0}\) is any particular lattice site which we define as the origin of the lattice. Because of the symmetry of the lattice, the inner product in the definition of \(\lambda\) will be the same irrespective of which \({\bf a}\) we choose among the \(\mathcal{Z}\) choices.
...
We define the time-domain Greens function between two points \({\bf r}\) and \({\bf r} + {\bf r}\) as
\begin{equation}\begin{aligned}
	G_\text{H-H}({\bf r}, t) = -i\theta(t) \braket{\Psi_\text{gs} | \left\{ c({\bf r} + r,t), c^\dagger({\bf r}) \right\} | \Psi_\text{gs}}~.
\end{aligned}\end{equation}
where the bulk Hamiltonian \(H_\text{H-H}\) leads to the dynamics of the annihilation operator: \(c({\bf r} + r,t) = e^{i H_\text{H-H} t}c({\bf r} + r)e^{-i H_\text{H-H} t}\). Following eq.~\ref{siam_to_hubb}, the exponential can be written as:
\begin{equation}\begin{aligned}
