...
A similar decomposition can be used to map the self-energies of the lattice model to those of the impurity model. We approximate the full \(k-\)space self-energy as
\begin{equation}\begin{aligned}
	\label{selfenergykspace}
	\Sigma(\vec k,\omega) \simeq \Sigma_\text{loc}(\omega) + \xi_{\vec k}\Sigma_\text{near-n}(\omega)~,
\end{aligned}\end{equation}
where \(\Sigma_\text{loc}(\omega) = \Sigma_\text{H-H}(\vec r=0,\omega)\) is the local self-energy and \(\Sigma_\text{near-n}(\omega) = \Sigma_\text{H-H}(\vec r=a, \omega)\) is the self-energy arising from processes that connect a site with its nearest neighbour. Following Dyson's equation, these self-energies can be written in terms of the real space Greens functions:
\begin{equation}\begin{aligned}
	\Sigma_\text{loc}(\omega) = \left( G_\text{loc}^{(0)} \right) ^{-1} - \left( G_\text{loc} \right) ^{-1} = \lambda_{\vec k_0}^2\left[\left( \mathcal{G}_{dd}^{(0)} \right) ^{-1} - \left( \mathcal{G}_{dd} \right) ^{-1}\right]~,
\end{aligned}\end{equation}
