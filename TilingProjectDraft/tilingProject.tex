...
\subsection{Comments on the nature of the metal near the transition}
...
\subsection{Low-energy effective theory for gapless excitations near the transition}
As the bath interaction \(W\) is tuned through the L-PG phase, the nodal region is the last to decouple from the impurity. This allows us to write down a simpler Kondo model near the transition, where only the nodal region is hybridising with the impurity spin through Kondo interactions. This is done by retaining only those scattering processes that originate from (or end at) the nodal point, \(\vec k_N = \left(\pi/2, \pi/2\right)\). The other leg of the scattering vertices will involve \(k-\)space points in the immediate vicinity of the node (\(\vec k_N + \vec q, |\vec q| \ll \pi\)), since these are the strongest. For these points, the Kondo terms can be written as
\begin{widetext}
\begin{equation}\begin{aligned}
	\tilde H_\text{imp-cbath} = \frac{1}{2}\sum_{\vec q, |\vec q| \ll \pi}\sum_{\alpha,\beta} J^*(\vec k_N, \vec k_N + \vec q)\vec{S}_d\cdot\vec{\sigma}_{\alpha\beta}\left(c^\dagger_{\vec k_N,\alpha}c_{\vec k_N + \vec q,\beta} + c^\dagger_{\vec k_N + \vec q,\alpha}c_{\vec k_N,\beta}\right) ~,
\end{aligned}\end{equation}
where the scattered momentum \(\vec k_N + \vec q\) resides in a small ball around the node \(\vec k_N\). \(J^*(\vec k_N, \vec k_N + \vec q)\) is the low-energy fixed point value of the coupling for such scattering processes. Throughout the rest of this section, we use the shorthand \(J^*(\vec q)\) to represent \(J^*(\vec k_N, \vec k_N + \vec q)\).

From symmetry considerations, we need to consider other processes as well: the Kondo term changes sign under \(\vec k_N \to \vec k_N + \vec Q\) in one leg of the vertex (where \(\vec Q = \left( -\pi, -\pi \right) \), and remains invariant under transformation of both legs. This leads to three more classes of terms in the Hamiltonian:
\begin{equation}\begin{aligned}
	\tilde H_\text{imp-cbath} =& \frac{1}{2}\sum_{\vec q,\alpha,\beta}J^*(\vec q)\vec{S}_d\cdot\vec{\sigma}_{\alpha\beta}\left(c^\dagger_{\vec k_N,\alpha}c_{\vec k_N + \vec q,\beta} + c^\dagger_{\vec k_N + \vec q,\alpha}c_{\vec k_N,\beta} + c^\dagger_{\vec k_N + \vec Q,\alpha}c_{\vec k_N + \vec Q + \vec q,\beta} + c^\dagger_{\vec k_N + \vec Q + \vec q,\alpha}c_{\vec k_N + \vec Q,\beta}\right) ~,\\
			   & - \frac{1}{2}\sum_{\vec q}\sum_{\alpha,\beta}J^*(\vec q)\vec{S}_d\cdot\vec{\sigma}_{\alpha\beta}\left(c^\dagger_{\vec k_N + \vec Q,\alpha}c_{\vec k_N + \vec q,\beta} + c^\dagger_{\vec k_N + \vec q,\alpha}c_{\vec k_N + \vec Q,\beta} + c^\dagger_{\vec k_N,\alpha}c_{\vec k_N + \vec Q + \vec q,\beta} + c^\dagger_{\vec k_N + \vec Q + \vec q,\alpha}c_{\vec k_N,\beta}\right) ~.
\end{aligned}\end{equation}
\end{widetext}

In order to simplify the Hamiltonian, we define new fermionic operators
\begin{equation}\begin{aligned}
	a_{\vec k, \sigma} = \frac{1}{\sqrt 2}\left(c_{\vec k,\sigma} + c_{\vec k + \vec Q, \sigma}\right)~,
	b_{\vec k, \sigma} = \frac{1}{\sqrt 2}\left(c_{\vec k,\sigma} - c_{\vec k + \vec Q, \sigma}\right)~,
\end{aligned}\end{equation}
where \(\vec k\) is the momentum quantum number (chosen to lie in a small window around \(\left( \pi/2,\pi/2 \right) \)) and \(\sigma\) is the spin quantum number. That the operators are fermionic can be easily verified:
\begin{equation}\begin{aligned}
	\left\{ a_{\vec k,\sigma}, a^\dagger_{\vec k^\prime,\sigma^\prime} \right\} &= \left[\delta_{\vec k, \vec k^\prime} + \frac{1}{2}\left(\delta_{\vec k + \vec Q, \vec k^\prime} + \delta_{\vec k, \vec k^\prime + \vec Q}\right)  \right] \delta_{\sigma,\sigma^\prime} \\
	&= \delta_{\vec k, \vec k^\prime}\delta_{\sigma,\sigma^\prime}~,\\
	\left\{ b_{\vec k,\sigma}, b^\dagger_{\vec k^\prime,\sigma^\prime} \right\} &= \left[\delta_{\vec k, \vec k^\prime} - \frac{1}{2}\left(\delta_{\vec k + \vec Q, \vec k^\prime} + \delta_{\vec k, \vec k^\prime + \vec Q}\right)  \right] \delta_{\sigma,\sigma^\prime} \\
	&= \delta_{\vec k, \vec k^\prime}\delta_{\sigma,\sigma^\prime}~,\\
	\left\{ a_{\vec k,\sigma}, b^\dagger_{\vec k^\prime,\sigma^\prime} \right\} &= \left\{ b_{\vec k,\sigma}, a^\dagger_{\vec k^\prime,\sigma^\prime} \right\} = 0~,
\end{aligned}\end{equation}
where both \(\vec k\) and \(\vec k^\prime\) are chosen from a small window around the node, so their difference is also small and it is therefore guaranteed that \(|\vec k - \vec k^\prime| \neq |\vec Q|\), rendering both \(\delta_{\vec k + \vec Q, \vec k^\prime}\) and \(\delta_{\vec k, \vec k^\prime + \vec Q}\) zero. For convenience, we define the new number operators
\begin{equation}\begin{aligned}
	N_{\vec k,\sigma} = a^\dagger_{\vec k}a_{\vec k},~ \bar N_{\vec k,\sigma} = b^\dagger_{\vec k}b_{\vec k}~.
\end{aligned}\end{equation}

We have the following useful relation between the corresponding number operators:
\begin{equation}\begin{aligned}\label{numberOperatorRelation}
	\hat n_{\vec k,\sigma} + \hat n_{\vec k + \vec Q, \sigma} = N_{\vec k, \sigma} + \bar N_{\vec k,\sigma}~.
\end{aligned}\end{equation}

The Hamiltonian takes the following form in terms of these new degrees of freedom:
\begin{equation}\begin{aligned}
	&\tilde H_\text{imp-cbath} = \\
	&\sum_{\vec q,\alpha,\beta}J^*(\vec q)\vec{S}_d\cdot\vec{\sigma}_{\alpha\beta} \left(b^\dagger_{\vec k_N,\alpha}b_{\vec k_N + \vec q,\beta} + b^\dagger_{\vec k_N + \vec q,\alpha}b_{\vec k_N,\beta}\right)~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\tilde H_\text{cbath} &= \sum_{\vec q,\sigma}\varepsilon_{\vec k_N + \vec q}\left(\hat n_{\vec k_N + \vec q,\sigma} - \hat n_{\vec k_N + \vec Q + \vec q,\sigma}\right)\\
				&= \sum_{\vec q,\sigma}\varepsilon_{\vec k_N + \vec q}\left(a^\dagger_{\vec k_N + \vec q,\sigma} b_{\vec k_N + \vec q,\sigma} + b^\dagger_{\vec k_N + \vec q,\sigma} a_{\vec k_N + \vec q,\sigma}\right)~.
\end{aligned}\end{equation}

We also consider a simplified form of the bath interaction, taking into account the density-density correlations in \(k-\)space:
\begin{widetext}
\begin{equation}\begin{aligned}
	\tilde H_\text{cbath-int} =& -W\sum_{\vec q,\sigma}\left(\hat n_{\vec k + \vec q,\sigma} + \hat n_{\vec k + \vec Q + \vec q,\sigma}\right) + W\sum_{\vec q_1,\vec q_2}\left( \hat n_{\vec k_N + \vec q_1, \uparrow} + \hat n_{\vec k_N + \vec Q + \vec q_1, \uparrow} \right) \left( \hat n_{\vec k_N + \vec q_2, \downarrow} + \hat n_{\vec k_N + \vec Q + \vec q_2, \downarrow} \right)~,\\
	=& -W\sum_{\vec q,\sigma}\left(N_{\vec k_N + \vec q,\sigma} + \bar N_{\vec k_N + \vec q,\sigma}\right) + W\sum_{\vec q_1,\vec q_2}\left( N_{\vec k_N + \vec q_1,\uparrow} + \bar N_{\vec k_N + \vec q_1,\uparrow} \right) \left( N_{\vec k_N + \vec q_2,\downarrow} + \bar N_{\vec k_N + \vec q_2,\downarrow} \right)~.
\end{aligned}\end{equation}
\end{widetext}
We used eq.~\ref{numberOperatorRelation} in order to obtain the final form. Since the Hamiltonian is defined in a small window around the nodal point, the kinetic energy is the smallest term in the Hamiltonian. We therefore ignore it for the time-being, and address the two other terms. Since both \(\tilde H_\text{imp-cbath}\) and \(\tilde H_\text{cbath-int}\) commute with \(N_{\vec k, \sigma}\), we replace the local objects \(\sum_{\vec q}N_{\vec k_N + \vec q,\sigma}\) in \(\tilde H_\text{cbath-int}\) with \(\frac{1}{2}\) since the bath is always at half-filling. This results in a simplified Hamiltonian for just the \(b^\dagger\) degrees of freedom:
\begin{equation}\begin{aligned}
	\tilde H =& -\frac{1}{2}W\sum_{\vec q,\sigma}\bar N_{\vec k_N + \vec q,\sigma} + W\sum_{\vec q_1,\vec q_2}\bar N_{\vec k_N + \vec q_1,\uparrow} \bar N_{\vec k_N + \vec q_2,\downarrow}\\
			  &+ \sum_{\vec q,\alpha,\beta}J^*(\vec q)\vec{S}_d\cdot\vec{\sigma}_{\alpha\beta} \left(b^\dagger_{\vec k_N,\alpha}b_{\vec k_N + \vec q,\beta} + b^\dagger_{\vec k_N + \vec q,\alpha}b_{\vec k_N,\beta}\right)
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\mathcal{V} &= V + V^\dagger,\\
	V^\dagger &= \frac{1}{2}\sum_{\vec q}J^*(\vec q)\vec{S}_d^+ \left(b^\dagger_{\vec k_N,\downarrow}b_{\vec k_N + \vec q, \uparrow} + b^\dagger_{\vec k_N + \vec q,\downarrow}b_{\vec k_N, \uparrow}\right)~,
\end{aligned}\end{equation}
while the ``non-interacting" Hamiltonian is
\begin{equation}\begin{aligned}
	H_D &= -\frac{1}{2}W\sum_{\vec q,\sigma}\bar N_{\vec k_N + \vec q,\sigma} + W\sum_{\vec q_1,\vec q_2}\bar N_{\vec k_N + \vec q_1,\uparrow} \bar N_{\vec k_N + \vec q_2,\downarrow}~.
\end{aligned}\end{equation}
The renormalisation in the dynamics of \(b^\dagger_{\vec k,\sigma}\) are given by
\begin{equation}\begin{aligned}
	\Delta \tilde H &= \mathcal{P}_L \mathcal{V}^\dagger \mathcal{P}_H\frac{1}{E_L - H_D} \mathcal{V} \mathcal{P}_L\\
					&= \mathcal{P}_L \left[V \mathcal{P}_H\frac{1}{E_L - H_D} V^\dagger + V^\dagger \mathcal{P}_H\frac{1}{E_L - H_D} V \right]\mathcal{P}_L~,
\end{aligned}\end{equation}
where \(E_L\) is the energy of the ground state of \(H_D\), \(-(E_L - H_D)\) is the excitation energy of the state generated upon applying the perturbation \(V\), and \(\mathcal{P}_{L(H)}\) projects onto the low(high)-energy subspace. For the present Hamiltonian, the low-energy state (associated with \(E_L\)) is the one that minimises the bath interaction term \(\tilde H_\text{cbath-int}\); since the coupling \(W\) is negative, it projects out the spin sector of each \(k-\)state at low energies, leading to the following ground state:
\begin{equation}\begin{aligned}
	\ket{L} = \otimes_{\vec q}\frac{1}{\sqrt 2}\left(b^\dagger_{\vec k_N + \vec q, \uparrow}b^\dagger_{\vec k_N + \vec q, \downarrow} + 1\right) \ket{0}
\end{aligned}\end{equation}
where \(\ket{0}\) is the \(\bar N_{k \uparrow} = \bar N_{k \downarrow} = 0\) state. The state \(\ket{L}\) is at zero energy. High-energy states are obtained by applying, on the state \(\ket{L}\), the excitation operator \(b^\dagger_{\vec k_N,\sigma}b_{\vec k_N + \vec q,\bar\sigma}\) or its hermitian conjugate. The operator converts a double and a hole to spin states, incurring a total excitation cost of \(-W = |W|\).

We now calculate the various terms in \(\Delta \tilde H\):
\begin{widetext}
\begin{equation}\begin{aligned}
	&\mathcal{P}_L V^\dagger \mathcal{P}_H\frac{1}{E_L - H_D} V \mathcal{P}_L \\
	&= -\frac{{J^*(q)}^2}{4W}\left(\frac{1}{2} + S_d^z\right) \sum_{\vec q}\left(b^\dagger_{\vec k_N,\downarrow}b_{\vec k_N + \vec q, \uparrow} + b^\dagger_{\vec k_N + \vec q,\downarrow}b_{\vec k_N, \uparrow}\right) \left(b^\dagger_{\vec k_N,\uparrow}b_{\vec k_N + \vec q, \downarrow} + b^\dagger_{\vec k_N + \vec q,\uparrow}b_{\vec k_N, \downarrow}\right)\\
	&= -\frac{{J^*(q)}^2}{4W}\left(\frac{1}{2} + S_d^z\right) \sum_{\vec q}\left[\bar N_{\vec k_N, \downarrow}\left( 1 - \bar N_{\vec k_N + \vec q, \uparrow} \right) + \bar N_{\vec k_N + \vec q, \downarrow}\left( 1 - \bar N_{\vec k_N, \uparrow} \right) + b^\dagger_{\vec k_N, \downarrow} b^\dagger_{\vec k_N, \uparrow}b_{\vec k_N + \vec q, \downarrow} b_{\vec k_N + \vec q, \uparrow} + \text{h.c.}\right]~, \\
	&\mathcal{P}_L V \mathcal{P}_H\frac{1}{E_L - H_D} V^\dagger \mathcal{P}_L \\
	&= -\frac{{J^*(q)}^2}{4W}\left(\frac{1}{2} - S_d^z\right) \sum_{\vec q}\left[\bar N_{\vec k_N, \uparrow}\left( 1 - \bar N_{\vec k_N + \vec q, \downarrow} \right) + \bar N_{\vec k_N + \vec q, \uparrow}\left( 1 - \bar N_{\vec k_N, \downarrow} \right) + b^\dagger_{\vec k_N, \uparrow} b^\dagger_{\vec k_N, \downarrow}b_{\vec k_N + \vec q, \uparrow} b_{\vec k_N + \vec q, \downarrow} + \text{h.c.}\right] \\
\end{aligned}\end{equation}
\end{widetext}
Adding both contributions and setting \(S_d^z = 0\) owing to local SU(2) symmetry on the impurity site in the absence of fluctuations, we get
\begin{equation}\begin{aligned}
...
% \begin{figure}[htpb]
% 	\centering
% 	\includegraphics[width=0.8\textwidth]{tiledSF.pdf}
% 	\caption{tiledSF.pdf}
% 	\label{fig:tiledSF-pdf}
% \end{figure}
%
