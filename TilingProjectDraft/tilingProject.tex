...
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{C}}_{\left\{ \sigma_i \right\} }({\bf r}_3, {\bf r}_2, {\bf r}_1) \\
...
One can also look at \(k-\)space correlations, of the spin or charge kind. The spin-spin correlations are of the form
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{C}}_{\sigma_1,\sigma_2}({\bf K}_1, {\bf K}_2) \\
	&= \braket{c^\dagger_{\bar\sigma_1}\left({\bf K}_1\right)c_{\bar\sigma_2}^\dagger\left({\bf K}_2\right)c_{\sigma_2}\left({\bf K}_2\right)c_{\sigma_1}\left({\bf K}_1\right)}~.
\end{aligned}\end{equation}
We frame this in terms of the auxiliary model by (i) making the substitution again for the ground states, and (ii) by Fourier transforming the creation operators to real space and retaining just the impurity site operator:
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{C}}_{\sigma_1,\sigma_2}({\bf K}_1, {\bf K}_2) \\
	&= \langle\psi_\text{gs}({\bf r}_c) | c^\dagger_{\bar\sigma_1}\left({\bf r}_c\right)c_{\bar\sigma_2}^\dagger\left({\bf r}_c\right)c_{\sigma_2}\left({\bf K}_2\right)c_{\sigma_1}\left({\bf K}_1\right) | \psi_\text{gs}({\bf r}_c)\rangle~.
\end{aligned}\end{equation}
Of course, one can also Fourier transform the other operators, so one can average over both possibilities to symmetrise the expression:
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{C}}_{\sigma_1,\sigma_2}({\bf K}_1, {\bf K}_2) \\
	&= \frac{1}{2}\langle\psi_\text{gs}({\bf r}_c) | \left[c^\dagger_{\bar\sigma_1}\left({\bf r}_c\right)c_{\bar\sigma_2}^\dagger\left({\bf r}_c\right)c_{\sigma_2}\left({\bf K}_2\right)c_{\sigma_1}\left({\bf K}_1\right) \right.\\
	&\left.+ c^\dagger_{\bar\sigma_1}\left({\bf K}_1\right)c_{\bar\sigma_2}^\dagger\left({\bf K}_2\right)c_{\sigma_2}\left({\bf r}_c\right)c_{\sigma_1}\left({\bf r}_c\right) \right] | \psi_\text{gs}({\bf r}_c)\rangle~.
\end{aligned}\end{equation}
