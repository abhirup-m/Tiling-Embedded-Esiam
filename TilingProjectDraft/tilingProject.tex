...
\subsection{One-particle \(k-\)space spectral function}
\begin{figure}[htpb]
	\centering
...
\begin{equation}\begin{aligned}
	\pi G_R^\prime(\omega) = P\int_{-\infty}^{\infty}d\omega^\prime~\frac{G_R^{\prime\prime}(\omega^\prime)}{\omega^\prime - \omega}~.
\end{aligned}\end{equation}
The spectral function was defined above as \(-1/\pi\) times the imaginary part of the Greens function. Exchanging \(G_R^{\prime\prime}\) for \(A\) gives
\begin{equation}\begin{aligned}
	G_R^\prime(\omega) = -P\int_{-\infty}^{\infty}d\omega^\prime~\frac{A(\omega^\prime)}{\omega^\prime - \omega}~.
\end{aligned}\end{equation}
This provides a way of calculating the real part of the Greens function if the spectral function is already known.
