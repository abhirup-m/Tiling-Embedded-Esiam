...
Properties of the ground state can be probed using static correlation functions that are expectation values of two-particle excitation operators. A general correlation function in real space is of the form
\begin{equation}\begin{aligned}
	&\tilde{G}^{(2)}_{\left\{ \sigma_i \right\} }({\bf r}_3, {\bf r}_2, {\bf r}_1) \\
	&= \braket{c^\dagger_{\sigma_4}\left({\bf r}_c + {\bf r}_3\right)c_{\sigma_3}^\dagger\left({\bf r}_c + {\bf r}_2\right)c_{\sigma_2}\left({\bf r}_c + {\bf r}_1\right)c_{\sigma_1}\left({\bf r}_c\right)}~,
\end{aligned}\end{equation}
where the expectation value is in the ground state \(\ket{\Psi_\text{gs}}\). We replace the full ground state with the ground state of the auxiliary model at \({\bf r}_c\):
\begin{equation}\begin{aligned}
	\tilde{G}^{(2)}_{\left\{ \sigma_i \right\} }&({\bf r}_3, {\bf r}_2, {\bf r}_1) \\
	=& \langle\psi_\text{gs}({\bf r}_c) | c^\dagger_{\sigma_4}\left({\bf r}_c + {\bf r}_3\right)c_{\sigma_3}^\dagger\left({\bf r}_c + {\bf r}_2\right)c_{\sigma_2}\left({\bf r}_c + {\bf r}_1\right)\\
	&c_{\sigma_1}\left({\bf r}_c\right) | \psi_\text{gs}({\bf r}_c)\rangle~.
\end{aligned}\end{equation}

One can also look at \(k-\)space correlations, of the spin or charge kind. The spin-flip correlations are of the form
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{S}}_\text{flip}({\bf K}_1, {\bf K}_2) \\
	&= \frac{1}{2}\sum_{\sigma=\pm 1}\braket{c^\dagger_{\bar\sigma}\left({\bf K}_1\right)c_{\sigma}^\dagger\left({\bf K}_2\right)c_{\bar\sigma}\left({\bf K}_2\right)c_{\sigma}\left({\bf K}_1\right)}~.
\end{aligned}\end{equation}
We frame this in terms of the auxiliary model by (i) making the substitution again for the ground states, and (ii) by Fourier transforming the operators \(c^\dagger_{\bar\sigma}\left({\bf K}_1\right)\) and \(c_{\bar\sigma}\left({\bf K}_2\right)\) to real space and retaining just the impurity site operator:
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{S}}_\text{flip}({\bf K}_1, {\bf K}_2) \\
	&= \frac{1}{2}\sum_{\sigma=\pm 1}\langle\psi_\text{gs}({\bf r}_c) | c^\dagger_{\bar\sigma}\left({\bf r}_c\right)c_{\sigma}^\dagger\left({\bf K}_2\right)c_{\bar\sigma}\left({\bf r}_c\right)c_{\sigma}\left({\bf K}_1\right) | \psi_\text{gs}({\bf r}_c)\rangle~.
\end{aligned}\end{equation}
Of course, one can also Fourier transform the other operators (with spin \(\sigma\)), so one can average over both possibilities to symmetrise the expression:
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{S}}_\text{flip}({\bf K}_1, {\bf K}_2) \\
	&= \frac{1}{4}\sum_{\sigma=\pm 1}\langle\psi_\text{gs}({\bf r}_c) | \left[c^\dagger_{\bar\sigma}\left({\bf r}_c\right)c_{\sigma}^\dagger\left({\bf K}_2\right)c_{\bar\sigma}\left({\bf r}_c\right)c_{\sigma}\left({\bf K}_1\right) \right.\\
	& \quad \left. +~ c^\dagger_{\bar\sigma}\left({\bf K}_1\right)c_{\sigma}^\dagger\left({\bf r}_c\right)c_{\bar\sigma}\left({\bf K}_2\right)c_{\sigma}\left({\bf r}_c\right) \right] | \psi_\text{gs}({\bf r}_c)\rangle~.
\end{aligned}\end{equation}

In a similar fashion, one can probe nature of charge-fluctuation driven correlations using
\begin{equation}\begin{aligned}
	&\tilde{\mathcal{C}}_\text{flip}({\bf K}_1, {\bf K}_2) \\
	&= \frac{1}{2}\braket{\left[c^\dagger_{\uparrow}\left({\bf K}_2\right)c_{\downarrow}^\dagger\left({\bf K}_2\right)c_{\downarrow}\left({\bf K}_1\right)c_{\uparrow}\left({\bf K}_1\right) + \text{h.c.}\right] }~.
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
We again replace the states with the uniform part, while we expand the operators on the right of the density matrix (\(c\) and \(n\)) in a Fourier series in order to extract an impurity site operator:
\begin{equation}\begin{aligned}
	\overline\rho_{{\bf K},\sigma} = c^\dagger_{{\bf K},\sigma} \rho_\text{gs}({\bf r}_c) c_{{\bf r}_c,\sigma} + n_{{\bf K},\sigma} \rho_\text{gs}({\bf r}_c) n_{{\bf r}_c,\sigma}~.
\end{aligned}\end{equation}
The eigenvalues of this operator can be used to compute the entanglement entropy of the state \({\bf K}\sigma\).
