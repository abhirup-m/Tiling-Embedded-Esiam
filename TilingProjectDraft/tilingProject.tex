...
where 
\begin{itemize}
	\item \(\mathcal{H}_\text{cbath} = -\frac{1}{\mathcal{Z}}t\sum_{i=0,1,\ldots;\sigma}\left(c^\dagger_{i,\sigma}c_{i+1,\sigma} + \text{h.c.}\right) - \frac{1}{2}U_b\left(\hat n_{0 \uparrow} - \hat n_{0 \downarrow}\right)^2\) is the Hamiltonian of the mostly non-interacting conduction bath consisting of a kinetic energy term and some local interaction terms on the zeroth site,
...
\begin{equation}\begin{aligned}
	\mathcal{H}_\text{HH} &= -\frac{1}{\mathcal{Z}}\tilde t\sum_{\left<{\bf r}_i, {\bf r}_j\right>;\sigma}\left(c^\dagger_{{\bf r}_i,\sigma}c_{{\bf r}_j,\sigma} + \text{h.c.}\right) - \tilde \mu \sum_{{\bf r}}\hat n_{{\bf r},\sigma}\\
...
We define the time-domain Greens function between two quantum numbers \(\nu_1\) and \(\nu_2 \) at zero temperature as
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}\label{spectralRepresentation}
	&\tilde G(\nu_2, \nu_1; \tilde\omega) \\
	&= \frac{1}{\mathcal{D}_\text{gs}}\sum_{s_0 \in \text{GS}}\sum_s \left[\frac{F_p^*(s, \nu_2)F_p(s, \nu_1)}{\tilde\omega - \left( \tilde E_s - \tilde E_\text{gs} \right) } + \frac{F_h(s, \nu_1)F_h^*(s, \nu_2)}{\tilde\omega + \left( \tilde E_s - \tilde E_\text{gs} \right) }\right] ~,
\end{aligned}\end{equation}
where \(s_0=\left({\bf k}_0, n_0\right)\) sums over the ground state(s) of the system (with energy \(\tilde E_\text{gs}\) and degeneracy \(\mathcal{D}_\text{gs}\)) and \(s=\left({\bf k}, n\right)\) sums over all eigenstates (with energies \(\tilde E_s\)). The coefficients \(F_p(s, \nu) = \braket{\Psi_{s}|c^\dagger(\nu)|\Psi_{s_0}}\) and \(F_h(s, \nu) = \braket{\Psi_{s}|c(\nu)|\Psi_{s_0}}\) represent spectral weights for particle and hole excitations from the ground state \(\ket{s_0}\) into the excited state \(\ket{s}\). 
...
We first consider one-particle Greens functions for the propagation of excitations between two spatial points \(\nu_1={\bf r}_1\) and \(\nu_2={\bf r}_2\). Within the impurity model, the lattice site that most closely resembles the bulk lattice model is of course the impurity site; in order to ensure that all terms in eq.~\ref{spectralRepresentation} involve the impurity site, we approximate the states within \(F_p(s,\nu_1) = F_p(s,{\bf r}_1)\) of eq.~\ref{spectralWeightsAuxiliary} purely in terms of the auxiliary model at \({\bf r}_1\):
\begin{equation}\begin{aligned}
	\ket{\Psi_s} &= \frac{1}{\sqrt N}\sum_{{\bf r}_d} \left[e^{i {\bf k}\cdot{\bf r}_1} \ket{\psi_{n}\left({\bf r}_1\right)} + \ket{\psi_{n}}_\text{diff}\right]\\
		     &\simeq \sqrt N e^{i {\bf k}\cdot{\bf r}_1} \ket{\psi_{n}\left({\bf r}_1\right)}.
\end{aligned}\end{equation}
This amounts to retaining the spatially uniform part of the wavefunction and dropping the modulating difference term \(\ket{\psi_{n}}_\text{diff} = e^{i {\bf k}\cdot{\bf r}_d} \ket{\psi_{n}\left({\bf r}_d\right)} - e^{i {\bf k}\cdot{\bf r}_1} \ket{\psi_{n}\left({\bf r}_1\right)}\). Replacing the exact forms in \(F_p(s, {\bf r}_1)\) with this approximation results in the expression
\begin{equation}\begin{aligned}
	F_p(s, {\bf r}_1) \simeq N e^{-i \left({\bf k} - {\bf k}_0\right)\cdot{\bf r}_1} \braket{\psi_n({\bf r}_1)|c^\dagger({\bf r}_1)|\psi_{n_0}({\bf r}_1)}~.
\end{aligned}\end{equation}
For the other term \(F_p(s,\nu_2) = F_p(s,{\bf r}_2)\), we make the less drastic approximation of retaining the matrix elements between the same auxiliary models \({\bf r}_d = {\bf r}_d^\prime\), leading to
\begin{equation}\begin{aligned}
	F_p^*(s, {\bf r}_2) = \frac{1}{N}\sum_{{\bf r}_d} e^{i \left({\bf k}- {\bf k}_0\right)\cdot{\bf r}_d} \braket{\psi_{n_0}({\bf r}_d)|c({\bf r}_2)|\psi_{n}({\bf r}_d)}~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	...
	&= \braket{\psi_{n_0}({\bf r}_1)|T\left({\bf r}_d - {\bf r}_1\right) c({\bf r}_2) T^\dagger\left({\bf r}_d - {\bf r}_1\right) |\psi_{n}({\bf r}_1)}\\
	&= \braket{\psi_{n_0}({\bf r}_1)| c({\bf r}_2 + {\bf r}_1 - {\bf r}_d) |\psi_{n}({\bf r}_1)}~,
\end{aligned}\end{equation}
leading to the more useful form
\begin{equation}\begin{aligned}
	&F_p^*(s, {\bf r}_2) \\
	&= \frac{1}{N}\sum_{{\bf r}_d} e^{i \left({\bf k}- {\bf k}_0\right)\cdot{\bf r}_d} \braket{\psi_{n_0}({\bf r}_1)| c({\bf r}_2 + {\bf r}_1 - {\bf r}_d) |\psi_{n}({\bf r}_1)}\\
	&= \frac{1}{N}\sum_{{\bf r}_d} e^{i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_1 + {\bf r}_2 - {\bf r}_d\right)} \braket{\psi_{n_0}({\bf r}_1)| c({\bf r}_d) |\psi_{n}({\bf r}_1)}~.
\end{aligned}\end{equation}
In the second step, we redefined the dummy variable \({\bf r}_d \to {\bf r}_1 + {\bf r}_2 - {\bf r}_d\).
...
\begin{equation}\begin{aligned}
	F_h &= F_p^*\left({\bf k}\leftrightarrow{\bf k}_0, n \leftrightarrow n_0\right),\\
	F_h^* &= F_p\left({\bf k}\leftrightarrow{\bf k}_0, n \leftrightarrow n_0\right)~.
\end{aligned}\end{equation}
Using these, we get
\begin{equation}\begin{aligned}\label{spectralParticleHoleRelation}
	F_h &= \frac{1}{N}\sum_{{\bf r}_d} e^{i \left({\bf k}_0- {\bf k}\right)\cdot\left({\bf r}_1 + {\bf r}_2 - {\bf r}_d\right)} \braket{\psi_{n}({\bf r}_1)| c({\bf r}_d) |\psi_{n_0}({\bf r}_1)},\\
	F_h^* &= N e^{-i \left({\bf k}_0 - {\bf k}\right)\cdot{\bf r}_1} \braket{\psi_{n_0}({\bf r}_1)|c^\dagger({\bf r}_1)|\psi_{n}({\bf r}_1)}~.
\end{aligned}\end{equation}
...
\begin{widetext}
\begin{equation}\begin{aligned}
	\tilde G({\bf r}_2 - {\bf r}_1; \tilde\omega) &= \frac{1}{\mathcal{D}_\text{gs}}\sum_{n_0,{\bf k}_0}\sum_{{\bf k}, n}\sum_{{\bf r}_d}  \left[e^{i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_2 - {\bf r}_d\right)}\frac{\braket{\psi_{n_0}({\bf r}_1)| c({\bf r}_d) |\psi_{n}({\bf r}_1)} \braket{\psi_n({\bf r}_1)|c^\dagger({\bf r}_1)|\psi_{n_0}({\bf r}_1)}}{\tilde\omega - N\left( E_n - E_{n_0} + \varepsilon_{\bf k} - \varepsilon_{{\bf k}0}\right) } \right.\\
					  &\left.+ e^{-i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_2 - {\bf r}_d\right)}\frac{\braket{\psi_{n_0}({\bf r}_1)| c^\dagger({\bf r}_1) |\psi_{n}({\bf r}_1)} \braket{\psi_n({\bf r}_1)|c({\bf r}_d)|\psi_{n_0}({\bf r}_1)}}{\tilde\omega + N\left( E_n - E_{n_0} + \varepsilon_{\bf k} - \varepsilon_{{\bf k}0}\right) }\right] ~,\\
					  &= \frac{1}{N}\sum_{{\bf k}_0, {\bf k}}\sum_{{\bf r}_d}  \frac{1}{\mathcal{D}_\text{gs}}\sum_{n_0,n}\left[e^{i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_2 - {\bf r}_d\right)}\frac{\braket{\psi_{n_0}({\bf r}_1)| c({\bf r}_d) |\psi_{n}({\bf r}_1)} \braket{\psi_n({\bf r}_1)|c^\dagger({\bf r}_1)|\psi_{n_0}({\bf r}_1)}}{\omega - \left( E_n - E_{n_0} + \varepsilon_{\bf k} - \varepsilon_{{\bf k}0}\right) } \right.\\
					  &\left.+ e^{-i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_2 - {\bf r}_d\right)}\frac{\braket{\psi_{n_0}({\bf r}_1)| c^\dagger({\bf r}_1) |\psi_{n}({\bf r}_1)} \braket{\psi_n({\bf r}_1)|c({\bf r}_d)|\psi_{n_0}({\bf r}_1)}}{\omega + \left( E_n - E_{n_0} + \varepsilon_{\bf k} - \varepsilon_{{\bf k}0}\right) }\right] ~,
\end{aligned}\end{equation}
\end{widetext}
where \(\omega = \tilde\omega/N\) is the frequency scale for the auxiliary model. We can define partial Greens functions \(G_p({\bf r}_d, {\bf r}_1)\) and \(G_h({\bf r}_d, {\bf r}_1)\) for the auxiliary model that involve only the positive and negative frequency poles respectively. In terms of these, the lattice model Greens function can be written in a more intuitive form. Assuming a unique momentum \({\bf k}_0\) for the ground state and adjusting the ground state non-interacting energy \(\varepsilon_{{\bf k}0}\) to zero, we get
\begin{equation}\begin{aligned}\label{GreensFunctionRealSpace}
	&\tilde G({\bf r}_2 - {\bf r}_1; \tilde\omega) \\
	&= \frac{1}{N}\sum_{{\bf k}}\sum_{{\bf r}_d}  \left[e^{i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_2 - {\bf r}_d\right)}G_p\left({\bf r}_d,{\bf r}_1;\omega - \varepsilon_{\bf k}\right) \right.\\
	&\left.+ e^{-i \left({\bf k}- {\bf k}_0\right)\cdot\left({\bf r}_2 - {\bf r}_d\right)}G_h\left({\bf r}_d,{\bf r}_1;\omega + \varepsilon_{\bf k}\right)\right]
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	F_p^*(s, {\bf K}) = \frac{1}{N}\sum_{{\bf r}_d, {\bf r}_d^\prime} e^{i {\bf k}\cdot{\bf r}_d - {\bf k}_0\cdot{\bf r}_d^\prime} \braket{\psi_{n_0}({\bf r}_d^\prime)|c({\bf K})|\psi_{n}({\bf r}_d)}\\
	F_p(s, {\bf K}) = \frac{1}{N}\sum_{{\bf r}_d, {\bf r}_d^\prime} e^{-i {\bf k}\cdot{\bf r}_d + {\bf k}_0\cdot{\bf r}_d^\prime} \braket{\psi_n({\bf r}_d)|c^\dagger({\bf K})|\psi_{n_0}({\bf r}_d^\prime)}~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	&F_p^*(s, {\bf K}) = \frac{1}{N}\sum_{{\bf r}_d} e^{i \left({\bf k} - {\bf k}_0\right)\cdot{\bf r}_d} \braket{\psi_{n_0}({\bf r}_d)|c({\bf K})|\psi_{n}({\bf r}_d)}~,\\
...
\end{aligned}\end{equation}
With this, the final form of \(F_p^*(s, {\bf K})\) becomes
\begin{equation}\begin{aligned}
	F_p^*(s, {\bf K}) = \frac{1}{N}e^{i {\bf K}\cdot{\bf r}_c}\braket{\psi_{n_0}({\bf r}_c)|c({\bf K})|\psi_{n}({\bf r}_c)}\\
...
\begin{equation}\begin{aligned}
	F_p^*(s, {\bf K}) = e^{i {\bf K}\cdot{\bf r}_c}\delta_{{\bf K}, {\bf k} - {\bf k}_0}\braket{\psi_{n_0}({\bf r}_c)|c({\bf K})|\psi_{n}({\bf r}_c)}
\end{aligned}\end{equation}

For the other coefficient \(F_p(s, {\bf K})\), we again proceed similar to the previous subsection: we retain only the uniform part of the wavefunction, placing all the impurity models at the reference point \({\bf r}_c\):
\begin{equation}\begin{aligned}
	F_p(s, {\bf K}) = Ne^{-i \left({\bf k} - {\bf k}_0\right)\cdot{\bf r}_c} \braket{\psi_n({\bf r}_c)|c^\dagger({\bf K})|\psi_{n_0}({\bf r}_c)}~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	F_p(s, {\bf K}) = e^{-i \left({\bf k} - {\bf k}_0 + {\bf K}\right)\cdot{\bf r}_c} \braket{\psi_n({\bf r}_c)|c^\dagger({\bf r}_c)|\psi_{n_0}({\bf r}_c)}~.
\end{aligned}\end{equation}

Using eq.~\ref{spectralParticleHoleRelation}, we obtain the hole transition coefficients:
\begin{equation}\begin{aligned}
	F_h(s, {\bf K}) &= e^{i {\bf K}\cdot{\bf r}_c}\delta_{-{\bf K}, {\bf k} - {\bf k}_0}\braket{\psi_{n}({\bf r}_c)|c({\bf K})|\psi_{n_0}({\bf r}_c)},\\
	F_h^*(s, {\bf K}) &= e^{-i \left({\bf k}_0 - {\bf k} + {\bf K}\right)\cdot{\bf r}_c} \braket{\psi_{n_0}({\bf r}_c)|c^\dagger({\bf r}_c)|\psi_{n}({\bf r}_c)}~.
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
As a consistency check, one can Fourier transform the expression in eq.~\ref{GreensFunctionRealSpace} to recover the above expression.
