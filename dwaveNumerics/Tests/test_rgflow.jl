...
function test_high_low_separation_check()
...
function rgflow_check()
    @testset "RG flow" begin
        ...
        for W_val in [0.0, -J_val / 2, -1 * J_val]
            ...
            println(kondoJArray[3, 11, end])
            println(kondoJArray[3, 15, end])
