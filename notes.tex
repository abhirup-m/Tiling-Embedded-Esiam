...
\end{figure}
\\\\
The matrix elements for hole propagation are obtained similarly. Here the relavant excitations are $\ket{\overline{i\sigma}} \equiv c_{i\sigma}\ket{0}$, at frequency $-\omega$. To expand these states, we choose the $N-1$ eigenstates as the orthonormal basis:
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
The spectral weights remain unchanged; only the matrix element changes from $A_nm$ to $(A^{-1})_{nm}$. Inverting the matrix $\mathcal{G}_D$ is actually simple because it is diagonal in the chosen basis:
\begin{equation}\begin{aligned}
	\left(\mathcal{G}^{-1}_D\right)_{nn} = \begin{pmatrix} g_{0} & 0 & & \\ 0 & g_{1} & & \\ && .. &  \\ &&& g_{M} \end{pmatrix} \implies \left(\mathcal{G}_D\right)_{nn} = \begin{pmatrix} \frac{1}{g_0} & 0 & \\ 0 & \frac{1}{g_1} & \\ && ... & \\ &&& \frac{1}{g_M}\end{pmatrix}
\end{aligned}\end{equation}
