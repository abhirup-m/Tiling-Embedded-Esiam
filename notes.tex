...
\end{aligned}\end{equation}
This Hamiltonian is, however, explicitly asymmetric between the sites 0 and 1 (the effective bath couples only to 1). To rectify that, we will symmetrize the coupling between the effective bath and the sites 0 and 1, by exchanging the sites 0 and 1. The resultant Hamiltonian is
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	\sum_{\vec k}^\text{eff. bath} = c_z \equiv \left(\sum_{j \in \atop{\text{NN of 1}}} + \sum_{j \in \atop{\text{NN of 0}}}\right)c_{j}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\tilde H^D = H^D(0,1) - t^D \sum_{\vec k}^\text{eff. bath}\left(c^\dagger_{0\sigma}c_{\vec k,\sigma} + c^\dagger_{0\sigma}c_{\vec k,\sigma} + \text{h.c.}\right) + \sum_{\vec k}^\text{eff. bath}\epsilon_{\vec k}\hat n_{\vec k}
\end{aligned}\end{equation}
\(H^D(0,1)\) is the Hubbard dimer Hamiltonian:
\begin{equation}\begin{aligned}
	\label{dimer_ham}
...
\\\\
We  can thus obtain a family of Hamiltonians \(\tilde H^D(i,j)\) obtained by setting \(0\) and \(1\) by setting the two sites of the dimer to any nearest-neighbour \(i,j\) on the lattice and the effective bath to the rest \(N-2\) sites of the lattice. We will then create the the entire Hubbard Hamiltonian by combining the members of this family of Hamiltonians according to a prescription which is guided by the symmetries of the problem.
\begin{figure}[htpb]
	\centering
...
\end{figure}
\par\noindent
However, we first specify the two approximations made along the entire journey from the original Hubbard model to the reconstructed one:
\begin{itemize}
	...
	\item We then perform a unitary RG on $H^A$. This leads to a fixed-point Hamiltonian $\mathcal{U}_A H^A \mathcal{U}_A^\dagger$. At this point, we go from the fixed point Hamiltonian to the dimer+bath Hamiltonian: $H^D = Z\left[\mathcal{U}_A H^A \mathcal{U}_A^\dagger \right] Z^{-1}$. This constitutes the second approximations we make. This constitutes the second approximations we make.
\end{itemize}
...
\begin{equation}\begin{aligned}
	H^D_{i,j} = H^D(i, j) -t^A\sum_{k \in \text{NN of i}}\left(c^\dagger_{i\sigma}c_{k\sigma} + \text{h.c.}\right)-t^A\sum_{k \in \text{NN of j}}\left(c^\dagger_{j\sigma}c_{k\sigma} + \text{h.c.}\right) - t^A\sum_{\left<k,q\right> \neq i,j}\left(c^\dagger_{j\sigma}c_{k\sigma} + \text{h.c.}\right)
\end{aligned}\end{equation}
\(H^D(0 \to i, 1 \to j)\) is the Hubbard dimer Hamiltonian in eq.~\ref{dimer_ham} with the indices 0 and 1 replaced by \(i, j\) respectively. Note that this local Hamiltonian actually has all the hoppings of the full Hubbard model, but the correlation is present just on the sites \(i,j\). We havent transformed the dimer parameters \(U^D, t^D\) to the Hubbard parameters yet.
\\\\
