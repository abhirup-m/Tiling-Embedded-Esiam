...
\end{align}
\\\\
{\textbf{\color{red}{[CHECK]That $U_b$ does not actually give a zero mode, rather it gives an $N^2$ type piece.}}
\\\\
...
\begin{equation}\begin{aligned}
	\frac{2}{Nw}\sum_{\left<ij \right>} H^D(i,j,t^D, U^D) \simeq  \mathcal{U}_A H^H  \mathcal{U}_A^\dagger
\end{aligned}\end{equation}
$\mathcal{U}_A$ is the total unitary transformation that completes the URG starting from the bare cluster+bath Anderson model up to the fixed-point Hamiltonian. 
...
If the groundstate wavefunction of $H^H$ is $\ket{0}$, then the groundstate wavefunction of $\tilde H$ is $\ket{\tilde 0} =  \mathcal{U}_A\ket{0}$. The advantage of $\tilde H$ being unitarily connected to $H^H$ is that they will have the same single-particle Greens functions.
\begin{equation}\begin{aligned}
	\left(G_H\right)^\sigma_{ij}(t) \equiv -i\theta(t)\bra{0}\left\{c_{i\sigma},c^\dagger_{j\sigma} \right\}\ket{0} &= -i\theta(t)\bra{0} \mathcal{U}_A^\dagger\left\{  \mathcal{U}_A c_{i\sigma}  \mathcal{U}_A^\dagger  \mathcal{U}_A c^\dagger_{j\sigma}  \mathcal{U}_A^\dagger +  \mathcal{U}_A c^\dagger_{j\sigma}  \mathcal{U}_A^\dagger  \mathcal{U}_A c_{i\sigma}  \mathcal{U}_A^\dagger\right\} \mathcal{U}_A\ket{0}\\
...
\begin{equation}\begin{aligned}
	\left[\left(G_H\right)^{-1}\right]^\sigma_{ij} = \bra{i\sigma}\omega - H^H\ket{j\sigma} = \bra{i\sigma} \mathcal{U}_A^\dagger\left(\omega -  \mathcal{U}_A H^H  \mathcal{U}_A^\dagger\right)  \mathcal{U}_A\ket{j\sigma} = \bra{\tilde i\sigma}\left(\omega - \tilde H\right)\ket{\tilde j\sigma} = \left[\left(\tilde G\right)^{-1}\right]^\sigma_{\tilde i,\tilde j}
\end{aligned}\end{equation}
...
\\\\
In terms of the inverse Greens function, equation \ref{H_tiled} becomes
\begin{equation}\begin{aligned}
	\label{green_eq}
	\omega - \left(\tilde G\right)^{-1}(\omega) = \frac{2}{Nw}\sum_{\left<i,j\right>}\left[\omega - G_D^{-1}\left(i, j\right)\right]
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	...
	\left(\tilde G\right)^{-1}(\omega) = \frac{2}{Nw}\sum_{\left<i,j\right>}G_D^{-1}\left(\omega, i, j\right)
\end{aligned}\end{equation}
where $G_D^{-1}(i,j,t^D,U^D)$ is the Greens function (matrix) of the Hubbard dimer Hamiltonian with $i, j$ as the two sites.
\\\\
We will now calculate the single-particle site-diagonal and site-off-diagonal Greens functions. First lets consider the diagonal Greens function at $i^\text{th}$ site. We will extract this using the definition
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
\\\\
{\textbf{\color{red}{[CHECK]How should we connect the Hubbard dimers wavefunctions with the renormalised ground state $\ket{\tilde 0}$? If we use just the latter, then the RHS will not be the dimer's greens functions. If we use just the former, then the LHS will not be the Greens functions of $\tilde H$, and hence not of $H^H$}.}}
\\\\
The only terms that will contribute on the RHS of eq.~\ref{green_eq_final} are those that have $i$ in one of the indices. There will be $w$ such terms, because $i$ has $w$ nearest neighbours. Thus, the right hand side will be a sum of $w$ terms, each term being the inverse Greens function of a Hubbard dimer. Each term will be a real space local Greens function, and because of translational invariance, it will be the same on both sites. So we will represent the real space local inverse Greens function as $\left[G_D^{-1}(\omega)\right]_{00\sigma}$. The entire thing is thus $w$ times this local Greens function.
\begin{equation}\begin{aligned}
	\label{local_gf}
