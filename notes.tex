...
\begin{equation}\begin{aligned}
	\mathcal{\tilde G}(\omega) = \frac{1}{\omega - \tilde H}, && \mathcal{G}_D(\omega) = \frac{1}{\omega - H^D}
\end{aligned}\end{equation}
\textit{It has been assumed that the Hamiltonians have been normal ordered such that the ground state energy is 0.} Then, these are the same operators that appear in the appendix. However, before proceeding, we should note that even though eq.~\ref{H_tiled} used the indices $i,$, the correct indices are actually $\tilde i, \tilde j$, in light of the fact that operators get renormalised as $c_i \to \tilde c_i\equiv Uc_i$. With this in mind, we can write
\begin{equation}\begin{aligned}
	\label{green_eq}
...
\end{figure}
The matrix elements for hole propagation are obtained similarly:
\begin{equation}\begin{aligned}
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.4\textwidth]{./dimer_dispersion.png}
...
\begin{equation}\begin{aligned}
 ...
 \section*{Appendix: Relation between single-particle Greens function and the Greens function operator}
