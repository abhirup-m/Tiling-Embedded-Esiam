...
\begin{equation}\begin{aligned}
	\label{app2}
	\tilde H = \mathcal{U} H^H \mathcal{U}^{-1}
\end{aligned}\end{equation}
where $\mathcal{U}$ is some transformation that is either a unitary, or, at the very least, a similarity transformation.. The content of the above equation is that we expect the final tiled translationally invariant new (renormalised) Hubbard Hamiltonian to be connected to the bare parent Hubbard model through some unitary/similarity transformation $\mathcal{U}$.
...
\begin{equation}\begin{aligned}
	\frac{2}{Nw}\sum_{\left<ij \right>} H^D(i,j,t^D, U^D) \simeq  \mathcal{U} H^H  \mathcal{U}^{-1}
\end{aligned}\end{equation}
$\mathcal{U}$ is in general distinct from the unitary transformation that completes the URG starting from the bare cluster+bath Anderson model up to the fixed-point Hamiltonian. It is a larger transformation that involves three steps: the URG transformations $\mathcal{U}_A$, the extracting of the zero mode $Z$ and the tiling procedure outline in this section, $T$.
\begin{equation}\begin{aligned}
	\mathcal{U} = T Z \mathcal{U}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\left(G_H\right)^\sigma_{ij}(t) \equiv -i\theta(t)\bra{0}\left\{c_{i\sigma},c^\dagger_{j\sigma} \right\}\ket{0} &= -i\theta(t)\bra{0} \mathcal{U}^{-1}\left\{  \mathcal{U} c_{i\sigma}  \mathcal{U}^{-1}  \mathcal{U} c^\dagger_{j\sigma}  \mathcal{U}^{-1} +  \mathcal{U} c^\dagger_{j\sigma}  \mathcal{U}^{-1}  \mathcal{U} c_{i\sigma}  \mathcal{U}^{-1}\right\} \mathcal{U}\ket{0}\\
...
\begin{equation}\begin{aligned}
	\left[\left(G_H\right)^{-1}\right]^\sigma_{ij} = \bra{i\sigma}\omega - H^H\ket{j\sigma} = \bra{i\sigma} \mathcal{U}^{-1}\left(\omega -  \mathcal{U} H^H  \mathcal{U}^{-1}\right)  \mathcal{U}\ket{j\sigma} = \bra{\tilde i\sigma}\left(\omega - \tilde H\right)\ket{\tilde j\sigma} = \left[\left(\tilde G\right)^{-1}\right]^\sigma_{\tilde i,\tilde j}
\end{aligned}\end{equation}
...
\begin{itemize}
	...
	\item The overall best way of calculating $g_0$ and $g_1$ is by systematically improving the ground state wavefunctions. This can done by obtaining the wavefunction of a Hamiltonian that has not just for the dimer, but also a bath (with dispersion) that connects with both the sites of the dimer:
\begin{equation}\begin{aligned}
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\textwidth]{./dimer_dispersion.png}
	\caption{Hubbard dimer with dispersion that connects to both sites}
\end{figure}
\\\\
By systematically increasing the number of nearest neighbours, we can get better and better ground state wavefunctions, with which we can compute matrix elements, and hence $g_0$, $g_1$. What this means is that if we had just a Hubbard dimer wavefunction, then the impurity can at most hop onto its nearest neighbour, and then hop back, hence the wavefunction in eq.~\ref{guess}. However, with this additional hopping, the impurity can now now hop on to the nearest neighbour $z$, and then on to the next nearest neighbours. This leads to entanglement between the impurity site and the next nearest neighbour sites, which was missing in eq.~\ref{guess}. The introduction of the dispersion gives the hope that we can capture a spectral function of the three peak form, because we have previously seen such a spectral function in the fixed point of the URG analysis of the SIAM (the fixed point Hamiltonian was an Anderson molecule with dispersion).
\item Another improvement can be made by introducing a self-energy into the dispersion. This introduces correlation on sites beyond the impurity and its nearest neighbour. The Hamiltonian would be the same as eq.~\ref{dimer_dispersion}, with $\epsilon_k$ replaced by $\tilde \epsilon_k \equiv \epsilon_k + \Sigma(k,\omega)$. The self-energy $\Sigma$ will have to be chosen depending on the phase we want to look at. With a $\Sigma$ that is singular near the Fermi surface at $\omega \to 0$, the dispersion will become gapped and the phase will be insulating. In such a situation, eq.~\ref{guess} will be a very good approximation to the actual ground state. On the other hand, if we use a self energy that vanishes near the Fermi surface ($\Sigma(\omega) \sim \omega^2$), then we will end up in a metallic phase. Introducing the self energy therefore gives us a larger variety of wavefunctions and features to work with.
\end{itemize}
\newpage
