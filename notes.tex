...
\begin{equation}\begin{aligned}
	G_H(\vec r, \omega) = \frac{1}{\sqrt N}\sum_{\vec k} e^{i\vec{k}.\vec{r}}G_H(\vec k, \omega) &= \frac{1}{\sqrt N}\sum_{\vec k}e^{i\vec{k}.\vec{r}}\left(g_0 + g_1 \xi_{\vec k}\right)^{-1}\\
												     &= \frac{\sqrt N}{2}\sum_{\vec k}e^{i\vec{k}.\vec{r}}\left\{\left[G_{D}^{-1}(\omega)\right]_{00\sigma} + \frac{1}{w}\left[G_{D}^{-1}(\omega)\right]_{01\sigma}\xi_{\vec k}\right\}^{-1}
\end{aligned}\end{equation}
Thus, we can now compute the real space local Green's function $G_H(\vec r = 0, \omega)$ by summing over the momenta:
\begin{equation}\begin{aligned}
	G_H(\vec r = 0, \omega) = \frac{1}{\sqrt N}\sum_{\vec k} G_H(\vec k, \omega) = \frac{1}{\sqrt N}\sum_{\vec k}\left(g_0 + g_1 \xi_{\vec k}\right)^{-1}
\end{aligned}\end{equation}
