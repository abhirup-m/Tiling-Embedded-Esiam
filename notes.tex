...
The simplest test involves choosing $H^H = H^D$ and then calculating the momentum space Greens function at $k=0$, $G(k=0, \sigma)$, for the Hubbard dimer \(\left(N = 2, w = 1 \right)\). The discrete set of momenta are \(\left\{ \vec k_n \right\} = 0, \frac{\pi}{a}\). For $k=0$, we have
\begin{equation}\begin{aligned}
	\xi_{\vec k = 0} = 1
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\label{Gk_dim}
	G(k=0,\sigma) = \left(g_0 + g_1 \right)^{-1} - \left(\overline{g_0} + \overline{g_1} \right)^{-1}
\end{aligned}\end{equation}
We will now calculate $g_0$ and $g_1$ from the Greens function matrices of the Hubbard dimer. From the spectral representation of the Greens function matrix elements in eq.~\ref{G_mat_spec}, we obtain
\begin{equation}\begin{aligned}
