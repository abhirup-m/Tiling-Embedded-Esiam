...
To build eigenstates that satisfy Bloch's theorem, we approximate the eigenstates of the auxiliary models to highly localised forms \(\psi_{\text{aux}}\left( \vec r_d - \vec R_i, \vec r_{0} - \vec R_i \right) \) which involve only the impurity electron \(\vec r_d\) and the zeroth site \(\vec r_0\) of the auxiliary model. With this approximation, the wavefunction for each auxiliary model is neglible outside the lattice site \(\vec R_i\). The Bloch function can now be constructed~\cite{stoyanova}:
\begin{equation}\begin{aligned}
	\label{bloch-states}
	\Psi_{\vec k}(\vec r) = \sum_{i} e^{i \vec{k}\cdot\vec{R}_i} \psi_\text{aux}\left(\vec r_d - \vec R_i, \vec r_{0} - \vec R_i\right)
\end{aligned}\end{equation}
Applying the full Hamiltonian on this state gives
\begin{equation}\begin{aligned}
	H_\text{H-H} \Psi_{\vec k}(\vec r) = \sum_{\vec R_i}\sum_{j} \mathcal{H}_\text{aux,i}\left(\left\{\vec r_k - \vec R_i\right\}\right) e^{i \vec{k}\cdot\vec{R}_j} \psi_\text{aux}\left(\vec r_d - \vec R_j, \vec r_{0} - \vec R_j\right)
\end{aligned}\end{equation}
From the local nature of the wavefunctions, we have \(\mathcal{H}_\text{aux,i}\psi_\text{aux,j} = \delta_{ij} E \psi_\text{aux,j}\), where \(E\) is the energy eigenvalue of the auxiliary model. Substituting this gives
\begin{equation}\begin{aligned}
	H_\text{H-H} \Psi_{\vec k}(\vec r) = E\left(\left\{\vec r_k - \vec R_i\right\}\right) e^{i \vec{k}\cdot\vec{R}_j} \psi_\text{aux}\left(\vec r_d - \vec R_j, \vec r_{0} - \vec R_j\right) = E \Psi_{\vec k}(\vec r)
\end{aligned}\end{equation}
The states constructed in eq.~\ref{bloch-states} are therefore eigenstates of the full Hamiltonian with the same eigenvalue \(E\) as the auxiliary model.

That \(\Psi_{\vec k}\) transforms like Bloch functions can be easily verified:
\begin{equation}\begin{aligned}
	\Psi_{\vec k}(\vec r + \vec R) = \sum_{i} e^{i \vec{k}\cdot\vec{R}_i} \psi_\text{aux}\left(\vec r_d + \vec R - \vec R_i, \vec r_0 + \vec R - \vec R_{i}\right) = e^{i \vec{k}\cdot\vec{R}}\sum_{j} e^{i \vec{k}\cdot\vec{R}_j} \psi_\text{aux}\left(\vec r_d - \vec R_j, \vec r_{0} + \vec R - \vec R_j\right) = e^{i \vec{k}\cdot\vec{R}} \Psi_{\vec k}(\vec r)
\end{aligned}\end{equation}
In the last equation, we transformed \(\vec R_i \to \vec R_j = \vec R_i - \vec R\).

Within the auxiliary model wavefunction \(\psi_\text{aux}\left(\vec r_d - \vec R_i, \vec r_0 - \vec R_i\right) \), the role of the impurity is to imitate the effect of correlations, while the role of the zeroth site is to introduce non-locality. Since it is the impurity site (and in general, the cluster) that is designed to be the closest to the bulk model in terms of local correlation, we will extract on-site measures of correlation only from the cluster sites, and involve the bath site only in the non-local measures.
