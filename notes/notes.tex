...
We can use the auxiliary model eigenstates \(\psi_\text{aux}\left(\left\{\vec r_k - \vec R_i\right\} \right) \equiv \psi_\text{aux}\left(\vec R_i\right)\) to construct eigenstates of the bulk Hamiltonian. The following combination of the auxiliary model eigenstates satisfies Bloch's theorem~\cite{stoyanova}:
\begin{equation}\begin{aligned}
	\label{bloch-states}
	\Psi_{\vec k}\left(\left\{ \vec r_k \right\} \right) = \sum_{i} e^{i \vec{k}\cdot\vec{R}_i} \psi_\text{aux}\left(\vec R_i\right)
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\bra{\Psi_{\vec k^\prime}}H_\text{H-H} \ket{\Psi_{\vec k}} = \sum_{ij}e^{i\left(\vec{k}\cdot\vec{R}_i - \vec{k}^\prime\cdot\vec{R}_j\right)} \bra{\psi_\text{aux}\left(\vec R_i\right)} H_\text{H-H}\ket{\psi_\text{aux,j}} = \sum_{i}e^{i\left(\vec{k} - \vec k^\prime\right)\cdot\vec{R}_i}\sum_j e^{i\vec k^\prime\cdot\left(\vec R_i - \vec{R}_j\right)} \bra{\psi_\text{aux}\left(\vec R_i\right)} H_\text{H-H}\ket{\psi_\text{aux,j}}
\end{aligned}\end{equation}
We define \(\vec L = \vec R_i - \vec R_j\), and sum over \(\vec L\): 
\begin{equation}\begin{aligned}
\bra{\Psi_{\vec k^\prime}}H_\text{H-H} \ket{\Psi_{\vec k}} = \sum_{\vec R_i}e^{i\left(\vec{k} - \vec k^\prime\right)\cdot\vec{R}_i}\sum_{\vec L} e^{i\vec k^\prime\cdot\vec L} \bra{\psi_\text{aux}\left(\vec R_i\right)} H_\text{H-H}\ket{\psi_\text{aux}\left(\vec R_i- \vec L\right)}
\end{aligned}\end{equation}
The sum over \(\vec L\) can be split into two summations, one over the distance \(L\) and another over the directions \(\vec a(L)\). Also note that because of translation symmetry, the matrix element will depend only on the distance \(L\) and not the direction: \(M(L) \equiv \bra{\psi_\text{aux}\left(\vec R_i\right)} H_\text{H-H}\ket{\psi_\text{aux}\left(\vec R_i- \vec L\right)}\).
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	\Psi_{\vec k}\left(\left\{\vec r_k + \vec R\right\} \right) = \sum_{\vec R_i} e^{i \vec{k}\cdot\vec{R}_i} \psi_\text{aux}\left(\vec R_i - \vec R\right) = \sum_{\vec R_j} e^{i \vec{k}\cdot\left(\vec{R}_j + \vec R\right)} \psi_\text{aux}\left(\vec R_j\right) = e^{i \vec{k}\cdot\vec{R}} \Psi_{\vec k}\left(\vec r\right)
\end{aligned}\end{equation}
...
We define \(\ket{\Phi_0(\vec k)}\) as the ground state of the bulk model, and it has an expression in terms of the auxiliary model eigenstates: \(\ket{\Phi_0(\vec k)} = \sum_i e^{i \vec{k}\cdot\vec{R}_i} \psi_\text{aux}\left( \vec R_i \right) \). To obtain Greens functions, we start by calculating real-space matrix elements of the bulk and auxiliary Hamiltonians. The states that we will consider are local excitations on top of the ground state: \(\ket{i} \equiv c^\dagger_{i\sigma}\ket{\Phi_0\left( \vec k \right) }\). \(i\) represents a lattice site. Sandwiching eq.~\ref{siam_to_hubb} between \(\ket{i}\) and \(\bra{i}\), we get
\begin{equation}\begin{aligned}
	\label{diag_mat_ele_1}
	\braket{i | H_\text{H-H} | i} = \sum_{\vec R_j} \braket{\Phi_0\left( \vec k \right) | c_{i\sigma} \mathcal{H}_\text{aux}\left( \vec R_j \right) c^\dagger_{i\sigma} | \Phi_0\left( \vec k \right) } = \sum_{\vec R_j, \vec R_m, \vec R_n} e^{i\vec{k}\cdot\left(\vec{R}_m - \vec R_n\right)} \braket{\psi_\text{aux}\left( \vec R_n \right) | c_{i\sigma} \mathcal{H}_\text{aux}\left( \vec R_j \right) c^\dagger_{i\sigma}  | \psi_\text{aux}\left( \vec R_m \right)}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\ket{\psi_\text{aux}\left( \vec R_m \right)} = \ket{\psi_\text{aux}\left( \left\{\vec r_k - \vec R_m\right\} \right)} = \ket{\psi_\text{aux}\left( \left\{\vec r_k - \vec R_0 + \left(\vec R_0 - \vec R_m\right)\right\} \right)} = T\left( \vec R_0 - \vec R_m \right) \ket{\psi_\text{aux}\left( \left\{\vec r_k - \vec R_0\right\} \right)} \\
	= T\left(\vec R_0 - \vec R_m\right) \ket{\psi_\text{aux}\left(\vec R_0\right)}
\end{aligned}\end{equation}
\begin{equation}\begin{aligned}
	\ket{\psi_\text{aux}\left( \vec R_n \right)} = T\left(\vec R_0 - \vec R_n\right) \ket{\psi_\text{aux}\left(\vec R_0\right)}
\end{aligned}\end{equation}
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	...
	&= \sum_{\vec R_j, \vec R_m, \vec R_n} e^{i\vec{k}\cdot\left(\vec{R}_m - \vec R_n\right)} \braket{\psi_\text{aux}\left( \vec R_0 \right) | T^\dagger\left( \vec R_0 - \vec R_n \right) c_{i\sigma} T\left( \vec R_0 - \vec R_j \right)\mathcal{H}_\text{aux}\left( \vec R_0 \right)T^\dagger\left( \vec R_0 - \vec R_j \right) c^\dagger_{i\sigma}  T\left( \vec R_0 - \vec R_m \right)| \psi_\text{aux}\left( \vec R_0 \right)}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	T^\dagger\left( \vec R_0 - \vec R_n \right) c_{i\sigma} T\left( \vec R_0 - \vec R_j \right) &= T^\dagger\left( \vec R_0 - \vec R_n \right) c_{\sigma}\left( \vec R_i \right)  T^\dagger\left(\vec R_j - \vec R_0\right) \\
												    ...
												    &=T^\dagger\left(\vec R_j - \vec R_n\right) c_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)
\end{aligned}\end{equation}
There, we used \(T^\dagger(\vec R_1) T^\dagger (\vec R_2) = T^\dagger(\vec R_1 + \vec R_2)\). For the creation operator, we similarly get
\begin{equation}\begin{aligned}
	T^\dagger\left( \vec R_0 - \vec R_j \right) c^\dagger_{i\sigma} T\left( \vec R_0 - \vec R_m \right) = \left[T^\dagger\left( \vec R_0 - \vec R_m \right) c_{i\sigma} T\left( \vec R_0 - \vec R_j \right)\right]^\dagger = c^\dagger_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right) T\left(\vec R_j - \vec R_m\right)
\end{aligned}\end{equation}
Substituting these forms gives
\begin{equation}\begin{aligned}
	&\braket{i | H_\text{H-H} | i} \\
	&= \sum_{\vec R_j, \vec R_m, \vec R_n} e^{i\vec{k}\cdot\left(\vec{R}_m - \vec R_n\right)} \braket{\psi_\text{aux}\left( \vec R_0 \right) | T^\dagger\left(\vec R_j - \vec R_n\right) c_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\mathcal{H}_\text{aux}\left( \vec R_0 \right)c^\dagger_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right) T\left(\vec R_j - \vec R_m\right)| \psi_\text{aux}\left( \vec R_0 \right)}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	&\braket{i | H_\text{H-H} | i} \\
	=& \sum_{\vec R_j, \vec R_m, \vec R_n} e^{i\vec{k}\cdot\left(\vec{R}_m - \vec R_n\right)} \bra{\psi_\text{aux}\left( \vec R_0 \right)} \sum_{\vec k_2}\ket{\vec k_2}\bra{\vec k_2}T^\dagger\left(\vec R_j - \vec R_n\right) c_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\mathcal{H}_\text{aux}\left( \vec R_0 \right)c^\dagger_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\\
	 &\sum_{\vec k_1}T\left(\vec R_j - \vec R_m\right)\ket{\vec k_1}\bra{\vec k_1} \ket{\psi_\text{aux}\left( \vec R_0 \right)}
\end{aligned}\end{equation}
From the definition of the translation operators shown above, we get \(T\left(\vec R_j - \vec R_m\right)\ket{\vec k_1} = e^{i \vec{k}_1\cdot\left(\vec{R}_j - \vec R_m\right)}\ket{\vec k_1}\) and \(\bra{\vec k_2}T^\dagger\left(\vec R_j - \vec R_n\right) = \bra{\vec k_2}e^{-i \vec{k}_2\cdot\left(\vec{R}_j - \vec R_n\right)}\). Substituting this gives
\begin{equation}\begin{aligned}
	&\braket{i | H_\text{H-H} | i} \\
	=& \sum_{\vec R_j, \vec R_m, \vec R_n}\sum_{\vec k_1,\vec k_2} e^{i\vec{R}_j\cdot\left(\vec{k}_1 - \vec k_2\right)} e^{i\vec{R}_n\cdot\left(\vec{k}_2 - \vec k\right)} e^{i\vec{R}_m\cdot\left(\vec{k} - \vec k_1\right)} \bra{\psi_\text{aux}\left( \vec R_0 \right)} \ket{\vec k_2}\bra{\vec k_2} c_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\mathcal{H}_\text{aux}\left( \vec R_0 \right)c^\dagger_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\\
	 &\ket{\vec k_1}\bra{\vec k_1} \ket{\psi_\text{aux}\left( \vec R_0 \right)}\\
	=& \sum_{\vec R_j}\sum_{\vec k_1,\vec k_2} e^{i\vec{R}_j\cdot\left(\vec{k}_1 - \vec k_2\right)} \delta_{\vec{k},\vec k_2} \delta_{\vec{k},\vec k_1} \bra{\psi_\text{aux}\left( \vec R_0 \right)} \ket{\vec k_2}\bra{\vec k_2} c_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\mathcal{H}_\text{aux}\left( \vec R_0 \right)c^\dagger_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\ket{\vec k_1}\bra{\vec k_1} \ket{\psi_\text{aux}\left( \vec R_0 \right)}\\
	=& \sum_{\vec R_j} \braket{\psi_\text{aux}\left( \vec R_0 \right) | \vec k}\bra{\vec k} c_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\mathcal{H}_\text{aux}\left( \vec R_0 \right)c^\dagger_{\sigma}\left( \vec R_i - \vec R_j + \vec R_0\right)\ket{\vec k}\braket{\vec k | \psi_\text{aux}\left( \vec R_0 \right)}\\
\end{aligned}\end{equation}
