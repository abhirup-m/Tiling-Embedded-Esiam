...
W_by_J_arr = -1.0 .* [0, 50, 56, 58, 60] ./ size_BZ
orbitals = ("p", "p")
...
@showprogress for (i, savePath) in collect(enumerate(savePaths))
    ...
    resSpin = getCorrelations(size_BZ, dispersion, kondoJArray, 0.0, orbitals, corrSpinArray)
    resCharge = getCorrelations(size_BZ, dispersion, kondoJArray, W_val, orbitals, corrChargeArray; twoParticle=1)
